...
public class FinVoucherModelHeadApiController extends ControllerSupport {
  ...
  public String getFinVoucherModelHeadList(
      Pages pages, FinVoucherModelHeadCO finVoucherModelHeadCO) {
    // TODO: 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<FinVoucherModelHead> list =
        this.finVoucherModelHeadService.getDataObjects(pages, finVoucherModelHeadCO);

    return JsonTextUtil.getDataJson(
        JsonUtil.listToJson(list, "yyyy-MM-dd HH:mm:ss"), JsonUtil.objectToJson(pages), null);
  }
  ...
  public String editFinVoucherModelHead(
      @Valid FinVoucherModelHead finVoucherModelHead, BindingResult bindingResult) {
    // 参数校验
