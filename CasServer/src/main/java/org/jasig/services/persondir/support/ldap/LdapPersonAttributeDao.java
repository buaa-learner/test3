 ...
 *
 * <p>In the case of multi valued attributes a {@link java.util.List} is set as the value. <br>
 * <br>
 * Configuration:
 *
 * <table border="1">
 ...
 * @since uPortal 2.5
 */
public class LdapPersonAttributeDao extends AbstractQueryPersonAttributeDao<LogicalFilterWrapper>
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (this.setReturningAttributes && resultAttributeMapping != null) {
      this.searchControls.setReturningAttributes(
          resultAttributeMapping.keySet().toArray(new String[resultAttributeMapping.size()]));
    }
   ...
   */
  @Override
  protected LogicalFilterWrapper appendAttributeToQuery(
      LogicalFilterWrapper queryBuilder, String dataAttribute, List<Object> queryValues) {
  ...
  protected List<IPersonAttributes> getPeopleForQuery(
      LogicalFilterWrapper queryBuilder, String queryUserName) {
    ...
    final List<Map<String, List<Object>>> queryResults =
        this.ldapTemplate.search(this.baseDN, ldapQuery, this.searchControls, MAPPER);

    final List<IPersonAttributes> peopleAttributes =
        new ArrayList<IPersonAttributes>(queryResults.size());
   ...
   * @deprecated Set the property on the {@link SearchControls} and set that via {@link
  ...
  /**
   * Optional wrapper template for the generated part of the query. Use {0} as a placeholder for
   * where the generated query should be inserted.
   */
