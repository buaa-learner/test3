...
public class ArInvoiceLineWebController extends ControllerSupport {
  ...
  public String getArInvoiceLineList(Pages pages, ArInvoiceLineCO receiptLineCO, Model model) {
    ...
    // 分页查询数据
    List<ArInvoiceLine> receiptLineList =
        this.arInvoiceLineService.getArInvoiceLineListByHeadCode(pages, receiptLineCO);
    for (ArInvoiceLine receiptLine : receiptLineList) {
      ...
      receiptLine.setStandard(
          this.masterDataCommonService
              .getMdMaterialInfoCache(soLine.getMaterialCode())
              .getStandard());
  ...
  public String getSelectSOLineModal(
      Pages pages, SoLineCO soLineCO, ArInvoiceLineCO receiptLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    for (SoLine soLine : soLineList) {
      ...
      if (mdMaterial.getMaterialType().equals("MATERIAL")) {
        // 如果是物料则取入库数量
        soLine.setOutputQuantity(
            this.invOutputLineService.getOutputQuantityBySoLineCode(soLine.getSoLineCode()));
      } else {
        // 如果是服务则入库数量直接取采购数量
    ...
    List<ArInvoiceLine> receiptLineList =
        this.arInvoiceLineService.getArInvoiceLineListByHeadCode(pagesTemp, receiptLineCO);
    ...
    if (receiptLine != null && receiptLine.getInvoiceLineId() != null) {
      ...
      if (StringUtils.isNotBlank(invoiceSourceType)) {
        if (invoiceSourceType.equals("SO")) {
          ...
          // 订单已开票数量
          receiptLine.setMadeInvoiceQuantity(
              this.arInvoiceLineService.getMadeInvoiceQuantityBySoLine(
                  soLine.getSoLineCode(), receiptLine.getInvoiceLineId()));
      ...
      if (StringUtils.isNotBlank(invoiceSourceType)) {
        if (invoiceSourceType.equals("SO")) {
          ...
          // 订单已开票数量
          receiptLine.setMadeInvoiceQuantity(
              this.arInvoiceLineService.getMadeInvoiceQuantityBySoLine(soLine.getSoLineCode(), 0));
        }
      }

      receiptLine.setStandard(
          this.masterDataCommonService
              .getMdMaterialInfoCache(receiptLine.getMaterialCode())
              .getStandard());
    }
  ...
  @ResponseBody
  public String editArInvoiceLine(
      @Valid ArInvoiceLine receiptLine,
