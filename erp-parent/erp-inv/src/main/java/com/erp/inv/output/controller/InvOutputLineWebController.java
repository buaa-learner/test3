...
public class InvOutputLineWebController extends ControllerSupport {
  ...
  @Autowired private InvStockService invStockService;
  ...
  public String getInvOutputLineList(Pages pages, InvOutputLineCO invOutputLineCO, Model model) {
    ...
    // 分页查询数据
    List<InvOutputLine> invOutputLineList =
        this.invOutputLineService.getInvOutputLineListByOutputHeadCode(pages, invOutputLineCO);
    for (InvOutputLine invOutputLine : invOutputLineList) {
      ...
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(soLine.getMaterialCode());
  ...
  public String getSelectSOLineModal(
      Pages pages, SoLineCO soLineCO, InvOutputLineCO invOutputLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    List<InvOutputLine> invOutputLineList =
        this.invOutputLineService.getInvOutputLineListByOutputHeadCode(pagesTemp, invOutputLineCO);
    ...
    while (soLineIt.hasNext()) {
      ...
      // 剔除非物料的采购订单行
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(soLineTemp.getMaterialCode());
  ...
  public String getInvOutputLine(
      InvOutputLine invOutputLine, String outputSourceType, Model model) {
    // 查询要编辑的数据
    if (invOutputLine != null && invOutputLine.getOutputLineId() != null) {
      ...
      if (StringUtils.isNotBlank(outputSourceType)) {
        if (outputSourceType.equals("SO")) {
          ...
          // 获取销售订单行已出库数量
          Double outputedQuantity =
              this.invOutputLineService.getOutputedQuantityBySoLine(
                  invOutputLine.getOutputSourceLineCode(), invOutputLine.getOutputLineId());
      ...
      if (StringUtils.isNotBlank(outputSourceType)) {
        if (outputSourceType.equals("SO")) {
          if (StringUtils.isNotBlank(invOutputLine.getMaterialCode())) {
            // 获取物料规格
            MdMaterial mdMaterial =
                this.masterDataCommonService.getMdMaterialInfoCache(
                    invOutputLine.getMaterialCode());
            invOutputLine.setMaterialStandard(mdMaterial.getStandard());

            // 获取销售订单行已出库数量
            Double outputedQuantity =
                this.invOutputLineService.getOutputedQuantityBySoLine(
                    invOutputLine.getOutputSourceLineCode(), 0);
            invOutputLine.setOutputedQuantity(outputedQuantity);
          }
        }
      }
    ...
    if (invOutputLine != null && StringUtils.isNotBlank(invOutputLine.getMaterialCode())) {
      stockNumber =
          this.invStockService.getStockNumberByMaterialCode(invOutputLine.getMaterialCode());
    }
  ...
  @ResponseBody
  public String editInvOutputLine(
      @Valid InvOutputLine invOutputLine,
