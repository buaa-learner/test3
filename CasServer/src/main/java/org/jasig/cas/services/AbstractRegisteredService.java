...
public abstract class AbstractRegisteredService
   ...
   */
  @Transient private RegisteredServiceAttributeFilter attributeFilter = null;

  /**
   * Name of the user attribute that this service expects as the value of the username payload in
   * the validate responses.
   */
  @Column(name = "username_attr", nullable = true, length = 256)
  private String usernameAttribute = null;

  /**
   * The logout type of the service. As front channel SLO is an experimental feature, the default
   * logout type is the back channel one.
   */
  @Transient private LogoutType logoutType = LogoutType.BACK_CHANNEL;
   ...
   * registered service.
   *
   * <p>Note: The username attribute will have no affect on services that are marked for anonymous
   * access.
   *
   * @param username attribute to release for this service that may be one of the following values:
   *     <ul>
   *       <li>name of the attribute this service prefers to consume as username.
   *       <li><code>null</code> to enforce default CAS behavior
   *     </ul>
   *
  ...
  /**
   * {@inheritDoc} Compares this instance with the <code>other</code> registered service based on
  ...
  public String toString() {
    final ToStringBuilder toStringBuilder =
        new ToStringBuilder(null, ToStringStyle.SHORT_PREFIX_STYLE);
