...
public class MdCustomerWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private MdCustomerService mdCustomerService;
  @Autowired private MdCustomerBankService mdCustomerBankService;
  @Autowired private MdCustomerContactService mdCustomerContactService;
  @Autowired private MdCustomerLicenseService mdCustomerLicenseService;
  ...
  public String editMdCustomer(
      @Valid MdCustomer mdCustomer,
  ...
  public String deleteMdCustomer(MdCustomer mdCustomer, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (mdCustomer != null
        && mdCustomer.getCustomerId() != null
        && StringUtils.isNotBlank(mdCustomer.getCustomerCode())) {
      int bankCount =
          this.mdCustomerBankService.getBankCountByCustomerCode(mdCustomer.getCustomerCode());
      int contactCount =
          this.mdCustomerContactService.getContactCountByCustomerCode(mdCustomer.getCustomerCode());
      int licenseCount =
          this.mdCustomerLicenseService.getLicenseCountByCustomerCode(mdCustomer.getCustomerCode());
