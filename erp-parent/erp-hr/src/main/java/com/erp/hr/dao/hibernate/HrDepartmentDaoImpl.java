...
public class HrDepartmentDaoImpl implements HrDepartmentDao {
  ...
  public List<HrDepartment> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, HrDepartmentCO paramObj) {
    return null;
  }
  ...
  public int getHrChildDepartmentNum(Integer departmentId) {
    String sql =
        "select count(*) from hr_department where parent_department_code = (select"
            + " d.department_code from hr_department d where d.department_id = :departmentId)";
  ...
  public List<HrDepartment> getHrDepartmentChildList(String departmentCode) {
    String sql =
        "select d.* from hr_department d where d.segment_code like (select concat(segment_code,'%')"
            + " from hr_department where department_code = :departmentCode) order by department_id";
