...
public class SimpleCacheStatistics implements CacheStatistics {
   ...
   * @param capacity Current cache capacity (e.g. items, bytes, etc). The units of capacity must be
   *     equal to size in order to produce a meaningful value for {@link #getPercentFree}.
  ...
  public SimpleCacheStatistics(
      final long size, final long capacity, final long evictions, final String name) {
    this.size = size;
    this.capacity = capacity;
    this.evictions = evictions;
    this.name = name;
