...
public class InvWarehouseWebController extends ControllerSupport {
  ...
  public String getInvWarehouseList(Pages pages, InvWarehouseCO invWarehouseCO, Model model) {
    ...
    // 分页查询数据
    List<InvWarehouse> invWarehouseList =
        this.invWarehouseService.getDataObjects(pages, invWarehouseCO);
  ...
  public String editInvWarehouse(
      @Valid InvWarehouse invWarehouse,
  ...
  public String deleteInvWarehouse(InvWarehouse invWarehouse, RedirectAttributes attr) {
    ...
    if (invWarehouse != null
        && invWarehouse.getWarehouseId() != null
        && StringUtils.isNotBlank(invWarehouse.getWarehouseCode())) {
      if (this.invWarehouseService.isExistRelateDataForInvWarehouse(
          invWarehouse.getWarehouseCode())) {
        // 提示信息
