...
/**
 * This servlet wraps the Spring DispatchServlet, catching any exceptions it throws on init() to
 * guarantee that servlet initialization succeeds. This allows our application context to succeed in
 * initializing so that we can display a friendly "CAS is not available" page to the deployer (an
 * appropriate use of the page in development) or to the end user (an appropriate use of the page in
 * production). The error page associated with this deployment failure is configured in the web.xml
 * via the standard error handling mechanism.
 *
 * <p>If the underlying Spring DispatcherServlet failed to init(), this SafeDispatcherServlet will
 * throw an <code>org.springframework.context.ApplicationContextException</code> on <code>service()
 * </code>.
 *
 * <p>The exception thrown by the underlying Spring DispatcherServlet init() and caught in the
 * SafeDispatcherServlet init() is exposed as a Servlet Context attribute under the key
...
public final class SafeDispatcherServlet extends HttpServlet {
   ...
   * {@inheritDoc}
   *
   * @throws ApplicationContextException if the DispatcherServlet does not initialize properly, but
