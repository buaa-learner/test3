...
/**
 * Concrete implementation of a CentralAuthenticationService, and also the central, organizing
 * component of CAS's internal implementation.
 *
 * <p>This class is threadsafe.
 *
 * <p>This class has the following properties that must be set:
 *
 * <ul>
 *   <li><code>ticketRegistry</code> - The Ticket Registry to maintain the list of available
 *       tickets.
 *   <li><code>serviceTicketRegistry</code> - Provides an alternative to configure separate
 *       registries for TGTs and ST in order to store them in different locations (i.e. long term
 *       memory or short-term)
 *   <li><code>authenticationManager</code> - The service that will handle authentication.
 *   <li><code>ticketGrantingTicketUniqueTicketIdGenerator</code> - Plug in to generate unique
 *       secure ids for TicketGrantingTickets.
 *   <li><code>serviceTicketUniqueTicketIdGenerator</code> - Plug in to generate unique secure ids
 *       for ServiceTickets.
 *   <li><code>ticketGrantingTicketExpirationPolicy</code> - The expiration policy for
 *       TicketGrantingTickets.
 *   <li><code>serviceTicketExpirationPolicy</code> - The expiration policy for ServiceTickets.
 * </ul>
 *
...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
  ...
  @NotNull private final TicketRegistry ticketRegistry;

  /**
   * New Ticket Registry for storing and retrieving services tickets. Can point to the same one as
   * the ticketRegistry variable.
   */
  @NotNull private final TicketRegistry serviceTicketRegistry;
  ...
  @NotNull private final UniqueTicketIdGenerator ticketGrantingTicketUniqueTicketIdGenerator;

  /** Map to contain the mappings of service->UniqueTicketIdGenerators. */
  @NotNull private final Map<String, UniqueTicketIdGenerator> uniqueTicketIdGeneratorsForService;

  /** Implementation of Service Manager. */
  @NotNull private final ServicesManager servicesManager;

  /** The logout manager. * */
  @NotNull private final LogoutManager logoutManager;

  /** Expiration policy for ticket granting tickets. */
  @NotNull private ExpirationPolicy ticketGrantingTicketExpirationPolicy;

  /** ExpirationPolicy for Service Tickets. */
  @NotNull private ExpirationPolicy serviceTicketExpirationPolicy;

  /** Encoder to generate PseudoIds. */
  @NotNull
  private PersistentIdGenerator persistentIdGenerator =
      new ShibbolethCompatiblePersistentIdGenerator();

  /**
   * The default attribute filter to match principal attributes against that of a registered
   * service. *
   */
  private RegisteredServiceAttributeFilter defaultAttributeFilter =
      new RegisteredServiceDefaultAttributeFilter();

  /**
   * Authentication policy that uses a service context to produce stateful security policies to
   * apply when authenticating credentials.
   */
  @NotNull
  private ContextualAuthenticationPolicyFactory<ServiceContext>
      serviceContextAuthenticationPolicyFactory = new AcceptAnyAuthenticationPolicyFactory();
  ...
  /**
   * {@inheritDoc} Destroy a TicketGrantingTicket and perform back channel logout. This has the
   * effect of invalidating any Ticket that was derived from the TicketGrantingTicket being
   * destroyed. May throw an {@link IllegalArgumentException} if the TicketGrantingTicket ID is
   * null.
   *
  ...
  public String grantServiceTicket(
    ...
    if (authns.size() > 1) {
      if (!registeredService.isAllowedToProxy()) {
        final String message =
            String.format(
                "ServiceManagement: Proxy attempt by service [%s] (registered service [%s]) is not"
                    + " allowed.",
    ...
    if (!this.uniqueTicketIdGeneratorsForService.containsKey(uniqueTicketIdGenKey)) {
      logger.warn(
          "Cannot create service ticket because the key [{}] for service [{}] is not linked to a"
              + " ticket id generator",
  ...
  /**
   * @throws IllegalArgumentException if the ServiceTicketId or the Credential are null.
   */
  @Audit(
  ...
  public Assertion validateServiceTicket(final String serviceTicketId, final Service service)
    ...
    try {
      ...
      Map<String, Object> attributesToRelease =
          this.defaultAttributeFilter.filter(
              principal.getId(), principal.getAttributes(), registeredService);
      if (registeredService.getAttributeFilter() != null) {
        attributesToRelease =
            registeredService
                .getAttributeFilter()
                .filter(principal.getId(), attributesToRelease, registeredService);
      }
  ...
  private String determinePrincipalIdForRegisteredService(
    ...
    if (registeredService.isAnonymousAccess()) {
      ...
      if (principal.getAttributes().containsKey(serviceUsernameAttribute)) {
        ...
        logger.warn(
            "Principal [{}] did not have attribute [{}] among attributes [{}] so CAS cannot provide"
                + " on the validation response the user attribute the registered service [{}]"
                + " expects. CAS will instead return the default username attribute [{}]",
            errorLogParameters);
      }
    }
