...
public class ApPayLineWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ApPayLineService apPayLineService;
  ...
  public String getApPayLineList(Pages pages, ApPayLineCO apPayLineCO, Model model) {
    ...
    // 分页查询数据
    List<ApPayLine> apPayLineList =
        this.apPayLineService.getPayLineListByHeadCode(pages, apPayLineCO);
    // 循环获取发票信息
    for (ApPayLine apPayLine : apPayLineList) {
      // 获取发票头信息
      ApInvoiceHead apInvoiceHead =
          this.apInvoiceHeadService.getDataObject(apPayLine.getInvoiceHeadCode());
      // 获取发票行汇总金额
      BigDecimal[] sumAmount =
          this.apInvoiceLineService.getInvoiceLineAmountSumByHeadCode(
              apPayLine.getInvoiceHeadCode());
  ...
  public String getSelectApInvoiceModal(
      Pages pages, ApInvoiceHeadCO apInvoiceHeadCO, ApPayLineCO apPayLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    List<ApInvoiceHead> invoiceHeadList =
        this.apInvoiceHeadService.getDataObjects(pages, apInvoiceHeadCO);
    // 循环设置发票信息
    for (ApInvoiceHead apInvoiceHead : invoiceHeadList) {
      apInvoiceHead.setReceiverName(
          this.masterDataCommonService.getVendorMap().get(apInvoiceHead.getReceiver()));
      // 设置税额
      BigDecimal[] sumAmount =
          this.apInvoiceLineService.getInvoiceLineAmountSumByHeadCode(
              apInvoiceHead.getInvoiceHeadCode());
    ...
    List<ApPayLine> payLineList =
        this.apPayLineService.getPayLineListByHeadCode(pagesTemp, apPayLineCO);
  ...
  public String getApPayLine(ApPayLine apPayLine, Model model) {
    ...
    if (apPayLine != null && apPayLine.getPayLineId() != null) {
      ...
      Double invoicePaidAmount =
          this.apPayLineService.getInvoicePaidAmount(
              apPayLine.getInvoiceHeadCode(), apPayLine.getPayLineId());
      ...
      // 获取发票已核销金额
      Double invoicePaidAmount =
          this.apPayLineService.getInvoicePaidAmount(apPayLine.getInvoiceHeadCode(), 0);
      apPayLine.setInvoicePaidAmount(invoicePaidAmount);
    }
  ...
  @ResponseBody
  public String editApPayLine(
      @Valid ApPayLine apPayLine,
