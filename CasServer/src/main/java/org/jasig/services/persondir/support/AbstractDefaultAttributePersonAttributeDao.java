...
/**
 * Abstract class implementing the IPersonAttributeDao method {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getPerson(String)} by delegation to {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getPeopleWithMultivaluedAttributes(Map)} using a
 * configurable default attribute name. If {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getPeopleWithMultivaluedAttributes(Map)}
 * returnes more than one {@link IPersonAttributes} is returned {@link
 * org.springframework.dao.IncorrectResultSizeDataAccessException} is thrown. <br>
 * <br>
 * Configuration:
 *
 * <table border="1">
...
public abstract class AbstractDefaultAttributePersonAttributeDao
   ...
   *     matching {@link IPersonAttributes} is found.
   */
  ...
  /**
   * Converts the uid to a multi-valued seed Map using the value from {@link
   * #getDefaultAttributeName()} as the key.
   */
  ...
  protected Map<String, List<Object>> toSeedMap0(
      String usernameAttributes, List<Object> username, List<Object> orgcode) {
  ...
  /**
   * The {@link IUsernameAttributeProvider} to use for determining the username attribute to use
   * when none is provided. The provider is used when calls are made to {@link #getPerson(String)}
