...
public class MdVendorDaoImpl implements MdVendorDao {
  ...
  public List<MdVendor> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, MdVendorCO paramObj) {
    return null;
  }
  ...
  public List<MdVendor> getMdVendorListForOwn() {
    String sql =
        "select c.* from md_vendor c where c.own_flag = 'Y' and c.status = 'Y' and c.approve_status"
            + " = 'APPROVE' order by c.vendor_id desc";
