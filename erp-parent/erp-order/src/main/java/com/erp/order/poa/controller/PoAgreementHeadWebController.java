...
public class PoAgreementHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private PoAgreementHeadService poAgreementHeadService;
  ...
  @Autowired private PoAgreementLineService poAgreementLineService;
  //    @Autowired
  ...
  public String getPoAgreementHeadList(
      Pages pages, PoAgreementHeadCO poAgreementHeadCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<PoAgreementHead> poAgreementHeadList =
        this.poAgreementHeadService.getDataObjectsForDataAuth("", pages, poAgreementHeadCO);
  ...
  public String getPoAgreementHead(PoAgreementHead poAgreementHead, Model model) {
    ...
    if (poAgreementHead != null && StringUtils.isNotBlank(poAgreementHead.getPoHeadCode())) {
      ...
      if (poAgreementHead.getVersion() != null && poAgreementHead.getVersion() > 0) {
        // poAgreementHead = this.poHeadMService.getDataObject(poAgreementHead.getPoHeadCode(),
        // poAgreementHead.getVersion());
      } else {
        poAgreementHead =
            this.poAgreementHeadService.getDataObject(poAgreementHead.getPoHeadCode());
      }

      // 设置显示字段
      poAgreementHead.setStaffName(
          this.hrCommonService.getHrStaff(poAgreementHead.getStaffCode()).getStaffName());
      poAgreementHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(poAgreementHead.getDepartmentCode())
              .getDepartmentName());
    } else {
      // 初始化默认字段
  ...
  public String editPoAgreementHead(
      @Valid PoAgreementHead poAgreementHead,
  ...
  public String deletePoAgreementHead(PoAgreementHead poAgreementHead, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (poAgreementHead != null
        && poAgreementHead.getPoHeadId() != null
        && StringUtils.isNotBlank(poAgreementHead.getPoHeadCode())) {
