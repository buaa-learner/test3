...
/**
 * Provides an authenticaiton manager that is inherently aware of multiple credentials and supports
 * pluggable security policy via the {@link AuthenticationPolicy} component. The authentication
 * process is as follows:
 *
 * <ul>
 *   <li>For each given credential do the following:
 *       <ul>
 *         <li>Iterate over all configured authentication handlers.
 *         <li>Attempt to authenticate a credential if a handler supports it.
 *         <li>On success attempt to resolve a principal by doing the following:
 *             <ul>
 *               <li>Check whether a resolver is configured for the handler that authenticated the
 *                   credential.
 *               <li>If a suitable resolver is found, attempt to resolve the principal.
 *               <li>If a suitable resolver is not found, use the principal resolved by the
 *                   authentication handler.
 *             </ul>
 *         <li>Check whether the security policy (e.g. any, all) is satisfied.
 *             <ul>
 *               <li>If security policy is met return immediately.
 *               <li>Continue if security policy is not met.
 *             </ul>
 *       </ul>
 *   <li>After all credentials have been attempted check security policy again. Note there is an
 *       implicit security policy that requires at least one credential to be authenticated. Then
 *       the security policy given by {@link #setAuthenticationPolicy(AuthenticationPolicy)} is
 *       applied. In all cases {@link AuthenticationException} is raised if security policy is not
 *       met.
 * </ul>
 *
...
public class PolicyBasedAuthenticationManager implements AuthenticationManager {
  ...
  /**
   * Default principal implementation that allows us to create {@link Authentication}s (principal
   * cannot be null).
   */
  ...
  @NotNull private AuthenticationPolicy authenticationPolicy = new AnyAuthenticationPolicy();

  /**
   * Map of authentication handlers to resolvers to be used when handler does not resolve a
   * principal.
   */
  @NotNull private final Map<AuthenticationHandler, PrincipalResolver> handlerResolverMap;

  /**
   * Creates a new authentication manager with a varargs array of authentication handlers that are
   * attempted in the listed order for supported credentials. This form may only be used by
   * authentication handlers that resolve principals during the authentication process.
   *
  ...
  /**
   * Creates a new authentication manager with a list of authentication handlers that are attempted
   * in the listed order for supported credentials. This form may only be used by authentication
   * handlers that resolve principals during the authentication process.
   *
  ...
  public PolicyBasedAuthenticationManager(final List<AuthenticationHandler> handlers) {
    ...
    this.handlerResolverMap =
        new LinkedHashMap<AuthenticationHandler, PrincipalResolver>(handlers.size());
  ...
  /**
   * Creates a new authentication manager with a map of authentication handlers to the principal
   * resolvers that should be used upon successful authentication if no principal is resolved by the
   * authentication handler. If the order of evaluation of authentication handlers is important, a
   * map that preserves insertion order (e.g. {@link LinkedHashMap}) should be used.
   *
   * @param map Non-null map of authentication handler to principal resolver containing at least one
   *     entry.
   */
  ...
  /**
   * Sets the authentication metadata populators that will be applied to every successful
   * authentication event.
   *
   * @param populators Non-null list of metadata populators.
   */
  public final void setAuthenticationMetaDataPopulators(
      final List<AuthenticationMetaDataPopulator> populators) {
   ...
   *
   * @param policy Non-null authentication policy. The default policy is {@link
   ...
   * @return An authentication containing a resolved principal and metadata about successful and
   *     failed authentications. There SHOULD be a record of each attempted authentication, whether
   *     success or failure.
   * @throws AuthenticationException When one or more credentials failed authentication such that
   *     security policy was not satisfied.
   */
  protected AuthenticationBuilder authenticateInternal(final Credential... credentials)
    ...
    for (final Credential credential : credentials) {
      ...
      if (!found) {
        logger.warn(
            "Cannot find authentication handler that supports {}, which suggests a configuration"
                + " problem.",
            credential);
      }
    }
  ...
  protected Principal resolvePrincipal(
    ...
    if (resolver.supports(credential)) {
      ...
      logger.warn(
          "{} is configured to use {} but it does not support {}, which suggests a configuration"
              + " problem.",
          handlerName,
          resolver,
          credential);
   ...
   * @return Authentication exception containing information about authentication successes and
   *     failures.
   */
  ...
  /**
   * Null prinicpal implementation that allows us to construct {@link Authentication}s in the event
   * that no principal is resolved during the authentication process.
   */
