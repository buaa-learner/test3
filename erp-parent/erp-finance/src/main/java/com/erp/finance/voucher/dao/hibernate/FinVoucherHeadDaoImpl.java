...
public class FinVoucherHeadDaoImpl implements FinVoucherHeadDao {
  ...
  public List<FinVoucherHead> getDataObjects(Pages pages, FinVoucherHeadCO paramObj) {
    ...
    sql =
        sql
            + DaoUtil.getSQLConditionForDateTime(
                paramObj, "voucherDate", "voucherStartDate", "voucherEndDate", "and v.", args);

    if (StringUtils.isNotBlank(paramObj.getBusinessType())) {
      sql =
          sql
              + " and exists(select 1 from fin_voucher_bill_r where bill_type = :billtype and"
              + " voucher_head_code = v.voucher_head_code) ";
  ...
  public List<FinVoucherHead> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, FinVoucherHeadCO paramObj) {
    String sql = "select v.* from fin_voucher_head v where 1=1";
  ...
  public void updateFinVoucherHeadForApproveStatus(Integer voucherHeadId, String status) {
    String sql =
        "update fin_voucher_head set status = :status where voucher_head_id = :voucherheadid";
  ...
  public void updateFinVoucherHeadForStatus(Integer voucherHeadId, String approveStatus) {
    String sql =
        "update fin_voucher_head set approve_status = :approvestatus where voucher_head_id ="
            + " :voucherheadid";
  ...
  public int getVoucherHeadNum(String startDate, String endDate) {
    String sql =
        "select count(*) from fin_voucher_head where created_date between :startDate and :endDate";
  ...
  public FinVoucherHead getVoucherHead(String billType, String billHeadCode) {
    String sql =
        "select h.* from fin_voucher_head h,fin_voucher_bill_r b "
