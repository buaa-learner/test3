 ...
 * {@link TicketRegistryState} interface for exposing internal state information used in status
 * reports.
 *
...
public class SessionMonitor implements Monitor<SessionStatus> {
  ...
  @NotNull private TicketRegistryState registryState;
  ...
  /**
   * Sets the threshold above which warnings are issued for service ticket counts in excess of
   * value.
   *
   * @param threshold Warn threshold if non-negative value, otherwise warnings are disabled.
   */
      ...
      if (this.sessionCountWarnThreshold > -1 && sessionCount > this.sessionCountWarnThreshold) {
        ...
        msg.append(
            String.format(
                ...
                sessionCount, this.sessionCountWarnThreshold));
      } else {
        msg.append(sessionCount).append(" sessions. ");
      }
      if (this.serviceTicketCountWarnThreshold > -1
          && ticketCount > this.serviceTicketCountWarnThreshold) {
        code = StatusCode.WARN;
        msg.append(
            String.format(
