...
/**
 * Concrete implementation of a TicketGrantingTicket. A TicketGrantingTicket is the global
 * identifier of a principal into the system. It grants the Principal single-sign on access to any
 * service that opts into single-sign on. Expiration of a TicketGrantingTicket is controlled by the
 * ExpirationPolicy specified as object creation.
 *
...
public final class TicketGrantingTicketImpl extends AbstractTicket implements TicketGrantingTicket {
  ...
  /**
   * Constructs a new TicketGrantingTicket. May throw an {@link IllegalArgumentException} if the
   ...
   */
  public TicketGrantingTicketImpl(
      final String id, final Authentication authentication, final ExpirationPolicy policy) {
   ...
   * {@inheritDoc}
   *
   * <p>The state of the ticket is affected by this operation and the ticket will be considered
   * used. The state update subsequently may impact the ticket expiration policy in that, depending
   * on the policy configuration, the ticket may be considered expired.
   */
  @Override
  public synchronized ServiceTicket grantServiceTicket(
    ...
    final ServiceTicket serviceTicket =
        new ServiceTicketImpl(
            id, this, service, this.getCountOfUses() == 0 || credentialsProvided, expirationPolicy);
