...
/**
 * Performs two important error handling functions on an {@link AuthenticationException} raised from
 * the authentication layer:
 *
 * <ol>
 *   <li>Maps handler errors onto message bundle strings for display to user.
 *   <li>Determines the next webflow state by comparing handler erors against {@link #errors} in
 *       list order. The first entry that matches determines the outcome state, which is the simple
...
public class AuthenticationExceptionHandler {
  ...
  @NotNull private List<Class<? extends Exception>> errors = DEFAULT_ERROR_LIST;

  /**
   * String appended to exception class name to create a message bundle key for that particular
   * error.
   */
  ...
  /**
   * Sets the message bundle prefix appended to exception class names to create a message bundle key
   ...
   * org.jasig.cas.authentication.AuthenticationException#getHandlerErrors()} with highest
   * precedence. Also sets an ERROR severity message in the message context of the form <code>
   * [messageBundlePrefix][exceptionClassSimpleName]</code> for each handler error that is
  ...
  public String handle(final AuthenticationException e, final MessageContext messageContext) {
    ...
    logger.trace(
        "Unable to translate handler errors of the authentication exception {}. Returning {} by"
            + " default...",
        e,
        messageCode);
    messageContext.addMessage(new MessageBuilder().error().code(messageCode).build());
