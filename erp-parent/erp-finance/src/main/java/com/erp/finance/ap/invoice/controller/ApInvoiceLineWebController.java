...
public class ApInvoiceLineWebController extends ControllerSupport {
  ...
  public String getApInvoiceLineList(Pages pages, ApInvoiceLineCO payLineCO, Model model) {
    ...
    // 分页查询数据
    List<ApInvoiceLine> payLineList =
        this.apInvoiceLineService.getApInvoiceLineListByHeadCode(pages, payLineCO);
    for (ApInvoiceLine payLine : payLineList) {
      ...
      payLine.setStandard(
          this.masterDataCommonService
              .getMdMaterialInfoCache(poLine.getMaterialCode())
              .getStandard());
  ...
  public String getSelectPOLineModal(
      Pages pages, PoLineCO poLineCO, ApInvoiceLineCO payLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    for (PoLine poLine : poLineList) {
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(poLine.getMaterialCode());
      if (mdMaterial.getMaterialType().equals("MATERIAL")) {
        // 如果是物料则取入库数量
        poLine.setInputQuantity(
            this.invInputLineService.getInputQuantityByPoLineCode(poLine.getPoLineCode()));
      } else {
        // 如果是服务则入库数量直接取采购数量
    ...
    List<ApInvoiceLine> payLineList =
        this.apInvoiceLineService.getApInvoiceLineListByHeadCode(pagesTemp, payLineCO);
    ...
    if (payLine != null && payLine.getInvoiceLineId() != null) {
      ...
      if (StringUtils.isNotBlank(invoiceSourceType)) {
        if (invoiceSourceType.equals("PO")) {
          ...
          // 订单已开票数量
          payLine.setMadeInvoiceQuantity(
              this.apInvoiceLineService.getMadeInvoiceQuantityByPoLine(
                  poLine.getPoLineCode(), payLine.getInvoiceLineId()));
      ...
      if (StringUtils.isNotBlank(invoiceSourceType)) {
        if (invoiceSourceType.equals("PO")) {
          ...
          // 订单已开票数量
          payLine.setMadeInvoiceQuantity(
              this.apInvoiceLineService.getMadeInvoiceQuantityByPoLine(poLine.getPoLineCode(), 0));
        }
      }

      payLine.setStandard(
          this.masterDataCommonService
              .getMdMaterialInfoCache(payLine.getMaterialCode())
              .getStandard());
    }
  ...
  @ResponseBody
  public String editApInvoiceLine(
      @Valid ApInvoiceLine payLine,
