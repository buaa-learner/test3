...
public abstract class AbstractServicesManagerMBean<T extends ServicesManager> {
  ...
  @NotNull private T servicesManager;
    ...
    for (final RegisteredService r : this.servicesManager.getAllServices()) {
      services.add(
          new StringBuilder()
              ...
              .append(r.isEnabled())
              .append(" ssoEnabled: ")
              .append(r.isSsoEnabled())
              .append(" serviceId: ")
              .append(r.getServiceId())
              .toString());
    }
  ...
  private void changeEnabledState(final long id, final boolean newState) {
    ...
    // we screwed up our APIs in older versions of CAS, so we need to CAST this to do anything
    // useful.
