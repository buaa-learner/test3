...
public class ApPayHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ApPayHeadService apPayHeadService;
  ...
  public String getApPayHead(ApPayHead payHead, Model model) {
    ...
    if (payHead != null && StringUtils.isNotBlank(payHead.getPayHeadCode())) {
      ...
      payHead.setDepartmentName(
          this.hrCommonService.getHrDepartment(payHead.getDepartmentCode()).getDepartmentName());
      // 获取供应商信息
      payHead.setVendorName(
          this.masterDataCommonService.getVendorMap().get(payHead.getVendorCode()));
    } else {
      // 初始化默认字段
  ...
  public String editApPayHead(
      @Valid ApPayHead apPayHead,
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("PAY", headCode);
      // 删除凭证
      ...
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode, new Double[] {voucherAmount.doubleValue()}, "PAY");
