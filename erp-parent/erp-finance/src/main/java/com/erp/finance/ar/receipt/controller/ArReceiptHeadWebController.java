...
public class ArReceiptHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ArReceiptHeadService arReceiptHeadService;
  ...
  public String getArReceiptHeadList(Pages pages, ArReceiptHeadCO arReceiptHeadCO, Model model) {
    ...
    // 分页查询数据
    List<ArReceiptHead> arReceiptHeadList =
        this.arReceiptHeadService.getDataObjects(pages, arReceiptHeadCO);
    ...
    if (receiptHead != null && StringUtils.isNotBlank(receiptHead.getReceiptHeadCode())) {
      ...
      receiptHead.setDepartmentName(
          this.hrCommonService
              ...
              .getDepartmentName());
      // 获取供应商信息
      receiptHead.setCustomerName(
          this.masterDataCommonService.getCustomerMap().get(receiptHead.getCustomerCode()));
    } else {
      // 初始化默认字段
  ...
  public String editArReceiptHead(
      @Valid ArReceiptHead arReceiptHead,
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("RECEIPT", headCode);
      // 删除凭证
      ...
      // 计算分录的金额
      // 获取入库行
      List<ArReceiptLine> lineList =
          this.arReceiptLineService.getArReceiptLineListByHeadCode(headCode);
      ...
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode, new Double[] {voucherAmount.doubleValue()}, "RECEIPT");
