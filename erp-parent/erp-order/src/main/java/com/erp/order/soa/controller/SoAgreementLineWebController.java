...
public class SoAgreementLineWebController extends ControllerSupport {
  ...
  public String getSoAgreementLineList(
      Pages pages, SoAgreementLineCO soAgreementLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    if (soAgreementLineCO.getVersion() != null && soAgreementLineCO.getVersion() > 0) {
      ...
      List<SoAgreementLine> list =
          this.soAgreementLineService.getSoLineListBySoHeadCode(pages, soAgreementLineCO);
  ...
  @ResponseBody
  public String editSoAgreementLine(
      @Valid @RequestBody SoAgreementLine soAgreementLine,
  ...
  @ResponseBody
  public String deleteSoAgreementLine(
      @RequestBody SoAgreementLine soAgreementLine, RedirectAttributes attr) {
    try {
      // 删除数据前验证数据
      if (soAgreementLine != null && soAgreementLine.getSoLineId() != null) {
        // 删除数据
