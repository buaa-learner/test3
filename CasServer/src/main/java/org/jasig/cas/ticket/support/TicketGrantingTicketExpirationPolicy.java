...
/**
 * Provides the Ticket Granting Ticket expiration policy. Ticket Granting Tickets can be used any
 * number of times, have a fixed lifetime, and an idle timeout.
 *
 * @author William G. Thompson, Jr.
 * @since 3.4.10
 */
public final class TicketGrantingTicketExpirationPolicy
    implements ExpirationPolicy, InitializingBean, Serializable {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(TicketGrantingTicketExpirationPolicy.class);
  ...
  /**
   * @param timeToKillInSeconds time for the ticket to stay active in seconds Convenient virtual
   *     property setter to set time in seconds.
   */
  ...
  public boolean isExpired(final TicketState ticketState) {
    ...
    if ((System.currentTimeMillis() - ticketState.getCreationTime()
      ...
      LOGGER.debug(
          "Ticket is expired because the time since creation is greater than"
              + " maxTimeToLiveInMilliSeconds");
      return true;
    }
    ...
    if ((System.currentTimeMillis() - ticketState.getLastTimeUsed() >= timeToKillInMilliSeconds)) {
      LOGGER.debug(
          "Ticket is expired because the time since last use is greater than"
