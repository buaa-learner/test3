...
public class SysRoleWebController extends ControllerSupport {
  ...
  @Autowired private SysAuthService sysAuthService;
  @Autowired private SysRoleAuthRService sysRoleAuthRService;
  ...
  public String editSysRole(
      @Valid SysRole sysRole, BindingResult bindingResult, Model model, RedirectAttributes attr) {
    // 参数校验
  ...
  public String beforeRelateSysRoleAuth(String roleCode, Model model) {
    ...
    if (StringUtils.isNotBlank(roleCode)) {
      // 获取用户已关联的权限
      List<SysAuth> sysAuthRelateList =
          this.sysAuthService.getRelateSysAuthListByRoleCode(roleCode);
  ...
  public String relateSysRoleAuth(
      String roleCode, String[] authCode, Model model, RedirectAttributes attr) {
