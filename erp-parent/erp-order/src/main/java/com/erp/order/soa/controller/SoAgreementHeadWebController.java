...
public class SoAgreementHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private SoAgreementHeadService soAgreementHeadService;
  ...
  @Autowired private SoAgreementLineService soAgreementLineService;
  //    @Autowired
  ...
  public String getSoAgreementHeadList(
      Pages pages, SoAgreementHeadCO soAgreementHeadCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<SoAgreementHead> soAgreementHeadList =
        this.soAgreementHeadService.getDataObjectsForDataAuth("", pages, soAgreementHeadCO);
  ...
  public String getSoAgreementHead(SoAgreementHead soAgreementHead, Model model) {
    ...
    if (soAgreementHead != null && StringUtils.isNotBlank(soAgreementHead.getSoHeadCode())) {
      ...
      if (soAgreementHead.getVersion() != null && soAgreementHead.getVersion() > 0) {
        // soAgreementHead = this.soHeadMService.getDataObject(soAgreementHead.getSoHeadCode(),
        // soAgreementHead.getVersion());
      } else {
        soAgreementHead =
            this.soAgreementHeadService.getDataObject(soAgreementHead.getSoHeadCode());
      }

      // 设置显示字段
      soAgreementHead.setStaffName(
          this.hrCommonService.getHrStaff(soAgreementHead.getStaffCode()).getStaffName());
      soAgreementHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(soAgreementHead.getDepartmentCode())
              .getDepartmentName());
    } else {
      // 初始化默认字段
  ...
  public String editSoAgreementHead(
      @Valid SoAgreementHead soAgreementHead,
  ...
  public String deleteSoAgreementHead(SoAgreementHead soAgreementHead, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (soAgreementHead != null
        && soAgreementHead.getSoHeadId() != null
        && StringUtils.isNotBlank(soAgreementHead.getSoHeadCode())) {
