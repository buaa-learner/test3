...
public class FinVoucherHeadWebController extends ControllerSupport {
  ...
  @Autowired private FinVoucherBillRService finVoucherBillRService;
  ...
  public String getFinVoucherHeadList(Pages pages, FinVoucherHeadCO finVoucherHeadCO, Model model) {
    ...
    // 分页查询数据
    List<FinVoucherHead> finVoucherHeadList =
        this.finVoucherHeadService.getDataObjectsForDataAuth("", pages, finVoucherHeadCO);
    // 循环获取凭证金额
    for (FinVoucherHead finVoucherHead : finVoucherHeadList) {
      finVoucherHead.setAmount(
          this.finVoucherLineService.getFinVoucherAmountByVoucherHeadCode(
              finVoucherHead.getVoucherHeadCode()));
    }
  ...
  public String getFinVoucherHead(FinVoucherHead finVoucherHead, Model model) {
    ...
    // 查询要编辑的数据
    if (finVoucherHead != null
        && finVoucherHead.getVoucherHeadId() != null
        && StringUtils.isNotBlank(finVoucherHead.getVoucherHeadCode())) {
      ...
      // 获取凭证行
      finVoucherLineList =
          this.finVoucherLineService.getFinVoucherLineListByVoucherHeadCode(
              finVoucherHead.getVoucherHeadCode());
      // 循环设置凭证头的dr和cr
      Double drAmount = 0D;
      Double crAmount = 0D;
    ...
    Map approveStatusMap = GlobalDataBox.getApproveStatusMap();
    // 获取凭证模板
    List<FinVoucherModelHead> finVoucherModelHeadList =
        this.finVoucherModelHeadService.getFinVoucherModelHeadListForCustom();
  ...
  public String editFinVoucherHead(
      @Valid FinVoucherHead finVoucherHead,
    ...
    finVoucherHead.setVoucherNumber(
        FinVoucherUtil.incrVoucherNumberCache(finVoucherHead.getVoucherType()).toString());
  ...
  public String editFinVoucherHeadStatus(
      FinVoucherHead finVoucherHead, Model model, RedirectAttributes attr) {
    if (finVoucherHead != null
        && finVoucherHead.getVoucherHeadId() != null
        && StringUtils.isNotBlank(finVoucherHead.getVoucherHeadCode())) {
      if (StringUtils.isNotBlank(finVoucherHead.getStatus())) {
        this.finVoucherHeadService.updateFinVoucherHeadForStatus(
            finVoucherHead.getVoucherHeadId(),
            finVoucherHead.getVoucherHeadCode(),
            finVoucherHead.getStatus());
        // 提示信息
        attr.addFlashAttribute("hint", "success");
      } else if (StringUtils.isNotBlank(finVoucherHead.getApproveStatus())) {
        this.finVoucherHeadService.updateFinVoucherHeadForApproveStatus(
            finVoucherHead.getVoucherHeadId(), finVoucherHead.getApproveStatus());
        // 提示信息
        attr.addFlashAttribute("hint", "success");
      }
    }
  ...
  public String deleteFinVoucherHead(FinVoucherHead finVoucherHead, RedirectAttributes attr) {
    ...
    if (finVoucherHead != null
        && finVoucherHead.getVoucherHeadId() != null
        && StringUtils.isNotBlank(finVoucherHead.getVoucherHeadCode())) {
      if (finVoucherHead.getApproveStatus().equals("UNSUBMIT")
          || finVoucherHead.getApproveStatus().equals("REJECT")) {
        // 删除数据
