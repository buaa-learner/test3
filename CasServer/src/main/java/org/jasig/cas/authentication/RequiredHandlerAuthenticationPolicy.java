...
/**
 * Authentication security policy that is satisfied iff a specified authentication handler
 * successfully authenticates at least one credential.
 *
...
public class RequiredHandlerAuthenticationPolicy implements AuthenticationPolicy {
  ...
  @NotNull private final String requiredHandlerName;
  ...
  /**
   * Sets the flag to try all credentials before the policy is satisfied. This flag is disabled by
   * default such that the policy is satisfied immediately upon the first credential that is
   * successfully authenticated by the required handler.
   *
  ...
  public boolean isSatisfiedBy(final Authentication authn) {
    ...
    if (this.tryAll) {
      credsOk =
          authn.getCredentials().size() == authn.getSuccesses().size() + authn.getFailures().size();
    }
