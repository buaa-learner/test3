...
/**
 * Implementation of an expiration policy that adds the concept of saying that a ticket can only be
 * used once every X millseconds to prevent misconfigured clients from consuming resources by doing
 * constant redirects.
 *
...
public final class ThrottledUseAndTimeoutExpirationPolicy
    implements ExpirationPolicy, Serializable {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(ThrottledUseAndTimeoutExpirationPolicy.class);
  ...
  public boolean isExpired(final TicketState ticketState) {
    ...
    if ((System.currentTimeMillis() - ticketState.getLastTimeUsed()
        >= this.timeToKillInMilliSeconds)) {
      LOGGER.debug(
          "Ticket is expired due to the time being greater than the timeToKillInMilliseconds");
      return true;
    }
