...
public class InvInputLineWebController extends ControllerSupport {
  ...
  public String getInvInputLineList(Pages pages, InvInputLineCO invInputLineCO, Model model) {
    ...
    // 分页查询数据
    List<InvInputLine> invInputLineList =
        this.invInputLineService.getInvInputLineListByInputHeadCode(pages, invInputLineCO);
  ...
  public String getSelectPOLineModal(
      Pages pages, PoLineCO poLineCO, InvInputLineCO invInputLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    List<InvInputLine> invInputLineList =
        this.invInputLineService.getInvInputLineListByInputHeadCode(pagesTemp, invInputLineCO);
    ...
    while (poLineIt.hasNext()) {
      ...
      // 剔除非物料的采购订单行
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(poLineTemp.getMaterialCode());
    ...
    if (invInputLine != null && invInputLine.getInputLineId() != null) {
      ...
      if (StringUtils.isNotBlank(inputSourceType)) {
        if (inputSourceType.equals("PO")) {
          ...
          // 获取采购订单行已入库数量
          Double inputedQuantity =
              this.invInputLineService.getInputedQuantityByPoLine(
                  invInputLine.getInputSourceLineCode(), invInputLine.getInputLineId());
      ...
      if (StringUtils.isNotBlank(inputSourceType)) {
        if (inputSourceType.equals("PO")) {
          if (StringUtils.isNotBlank(invInputLine.getMaterialCode())) {
            // 获取物料规格
            MdMaterial mdMaterial =
                this.masterDataCommonService.getMdMaterialInfoCache(invInputLine.getMaterialCode());
            invInputLine.setMaterialStandard(mdMaterial.getStandard());

            // 获取采购订单行已入库数量
            Double inputedQuantity =
                this.invInputLineService.getInputedQuantityByPoLine(
                    invInputLine.getInputSourceLineCode(), 0);
            invInputLine.setInputedQuantity(inputedQuantity);
          }
        }
      }
  ...
  @ResponseBody
  public String editInvInputLine(
      @Valid InvInputLine invInputLine,
