...
public class FinVoucherModelHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private FinVoucherModelHeadService finVoucherModelHeadService;
  @Autowired private FinVoucherModelLineService finVoucherModelLineService;
  @Autowired private DatasetCommonService datasetCommonService;
  @Autowired private HrCommonService hrCommonService;
  @Autowired private MasterDataCommonService masterDataCommonService;
  @Autowired private ApPayHeadService payHeadService;
  @Autowired private ApPayLineService payLineService;
  @Autowired private ArReceiptHeadService receiptHeadService;
  @Autowired private ArReceiptLineService receiptLineService;
  @Autowired private FinVoucherHeadService finVoucherHeadService;
  @Autowired private FinVoucherLineService finVoucherLineService;
  ...
  public String getFinVoucherModelHeadList(
      Pages pages, FinVoucherModelHeadCO finVoucherModelHeadCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<FinVoucherModelHead> finVoucherHeadList =
        this.finVoucherModelHeadService.getDataObjects(pages, finVoucherModelHeadCO);

    // 循环设置职员和组织信息
    for (FinVoucherModelHead finVoucherHead : finVoucherHeadList) {
      finVoucherHead.setStaffName(
          this.hrCommonService.getHrStaff(finVoucherHead.getStaffCode()).getStaffName());
      finVoucherHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(finVoucherHead.getDepartmentCode())
              .getDepartmentName());
    }
  ...
  public String getFinVoucherModelHead(FinVoucherModelHead finVoucherModelHead, Model model) {
    ...
    if (finVoucherModelHead != null
        && finVoucherModelHead.getVoucherHeadId() != null
        && StringUtils.isNotBlank(finVoucherModelHead.getVoucherHeadCode())) {
      finVoucherModelHead =
          this.finVoucherModelHeadService.getDataObject(finVoucherModelHead.getVoucherHeadId());
      // 设置显示字段
      finVoucherModelHead.setStaffName(
          this.hrCommonService.getHrStaff(finVoucherModelHead.getStaffCode()).getStaffName());
      finVoucherModelHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(finVoucherModelHead.getDepartmentCode())
              .getDepartmentName());
      // 获取凭证行
      finVoucherLineList =
          this.finVoucherModelLineService.getFinVoucherModelLineListByVoucherHeadCode(
              finVoucherModelHead.getVoucherHeadCode());
      // 循环设置设置科目
      for (FinVoucherModelLine line : finVoucherLineList) {
        line.setSubjectDesc(
            this.masterDataCommonService.getSubjectMap().get(line.getSubjectCode()));
      }
    } else {
      // 初始化默认字段
      ...
      List<String> businessTypeList =
          this.finVoucherModelHeadService.getFinVoucherModelHeadForBusinessType();
      // 循环过滤掉已创建的业务类型
  ...
  public String getSelectBillModal(
      String businessType,
      Pages pages,
      ApPayHeadCO payHeadCO,
      ArReceiptHeadCO receiptHeadCO,
      Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    ...
    if (businessType.equals("PAY")) {
      // 分页查询数据
      List<ApPayHead> payHeadList =
          this.payHeadService.getApPayHeadListForNotCreateVoucher(pages, payHeadCO);
      // 循环获取金额
      // 循环设置职员和组织信息
      for (ApPayHead payHead : payHeadList) {
        payHead.setStaffName(
            this.hrCommonService.getHrStaff(payHead.getStaffCode()).getStaffName());
        payHead.setDepartmentName(
            this.hrCommonService.getHrDepartment(payHead.getDepartmentCode()).getDepartmentName());
      }
      ...
      // 分页查询数据
      List<ArReceiptHead> receiptHeadList =
          this.receiptHeadService.getArReceiptHeadListForNotCreateVoucher(pages, receiptHeadCO);
      // 循环获取金额
      // 循环设置职员和组织信息
      for (ArReceiptHead receiptHead : receiptHeadList) {
        receiptHead.setStaffName(
            this.hrCommonService.getHrStaff(receiptHead.getStaffCode()).getStaffName());
        receiptHead.setDepartmentName(
            this.hrCommonService
                .getHrDepartment(receiptHead.getDepartmentCode())
                .getDepartmentName());
      }
  ...
  public String editFinVoucherModelHead(
      @Valid FinVoucherModelHead finVoucherModelHead,
      BindingResult bindingResult,
      Integer[] voucherLineId,
      String[] memo,
      String[] subjectCode,
      String[] drAmount,
      String[] crAmount,
    ...
    // 保存编辑的数据
    this.finVoucherModelHeadService.insertOrUpdateFinVoucherModel(
        finVoucherModelHead, finVoucherLineList);
    // 提示信息
  ...
  public String deleteFinVoucherModelHead(
      FinVoucherModelHead finVoucherModelHead, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (finVoucherModelHead != null
        && finVoucherModelHead.getVoucherHeadId() != null
        && StringUtils.isNotBlank(finVoucherModelHead.getVoucherHeadCode())) {
  ...
  @ResponseBody
  public String autoCreateVoucher(
      String billHeadCode,
      Double amount,
      String businessType,
      Model model,
    ...
    if (StringUtils.isNotBlank(billHeadCode)
        ...
        && amount != null
        && amount > 0
        && DataBox.getVoucherBusinessType().containsKey(businessType)) {
      try {
        // 调用自动创建方法
        return this.finVoucherModelHeadService.autoCreateVoucher(
            billHeadCode, amount, businessType);
      } catch (Exception e) {
  ...
  public String getVoucherTypeNumber(Model model) {
    ...
    for (Entry<String, String> temp : voucherTypeSet) {
      voucherNumberMap.put(
          temp.getKey(), FinVoucherUtil.getVoucherNumberCache(temp.getKey()).toString());
    }
  ...
  public String getVoucherEntryModal(String billType, String billHeadCode, Model model) {
    ...
    Double crAmount = 0D;
    // 获取分录头
    FinVoucherHead finVoucherHead =
        this.finVoucherHeadService.getVoucherHead(billType, billHeadCode);
    // 获取分录行
    List<FinVoucherLine> finVoucherLineList =
        this.finVoucherLineService.getVoucherLineList(billType, billHeadCode);
