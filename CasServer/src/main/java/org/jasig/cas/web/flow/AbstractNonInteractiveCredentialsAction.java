...
/**
 * Abstract class to handle the retrieval and authentication of non-interactive credential such as
 * client certificates, NTLM, etc.
 *
...
public abstract class AbstractNonInteractiveCredentialsAction extends AbstractAction {
  ...
  protected final Event doExecute(final RequestContext context) {
    ...
    if (isRenewPresent(context) && ticketGrantingTicketId != null && service != null) {
      ...
      try {
        ...
        logger.debug(
            "Attempted to generate a ServiceTicket using renew=true with different credential", e);
      }
    }

    try {
      WebUtils.putTicketGrantingTicketInRequestScope(
          context, this.centralAuthenticationService.createTicketGrantingTicket(credential));
  ...
  /**
   * Hook method to allow for additional processing of the response before returning a success
   * event.
   *
   * @param context the context for this specific request.
   * @param credential the credential for this request.
