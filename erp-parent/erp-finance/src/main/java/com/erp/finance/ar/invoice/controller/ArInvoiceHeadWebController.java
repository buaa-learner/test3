...
public class ArInvoiceHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ArInvoiceHeadService arInvoiceHeadService;
  @Autowired private ArInvoiceLineService arInvoiceLineService;
  ...
  @Autowired private MdVendorBankService mdVendorBankService;
  @Autowired private SoHeadService soHeadService;
  @Autowired private SoLineService soLineService;
  ...
  public String getArInvoiceHeadList(Pages pages, ArInvoiceHeadCO receiptHeadCO, Model model) {
    ...
    // 分页查询数据
    List<ArInvoiceHead> receiptHeadList =
        this.arInvoiceHeadService.getDataObjectsForDataAuth("", pages, receiptHeadCO);
    ...
    if (receiptHead != null && StringUtils.isNotBlank(receiptHead.getInvoiceHeadCode())) {
      ...
      if (receiptHead.getInvoiceSourceType().equals("SO")) {
        ...
        BigDecimal HISAmount =
            this.arInvoiceLineService.getHISArInvoiceAmountForSO(
                receiptHead.getInvoiceSourceHeadCode(), receiptHead.getInvoiceHeadCode());
        receiptHead.setReceiptSourceHeadHISAmount(HISAmount == null ? 0D : HISAmount.doubleValue());
      }
      // 获取收款人信息
      receiptHead.setPayerName(
          this.masterDataCommonService.getCustomerMap().get(receiptHead.getPayer()));
      // 获取银行信息
      receiptHead.setBankName(this.datasetCommonService.getBank().get(receiptHead.getBankCode()));
      // 获取客户相关信息
      try {
        MdCustomer mdCustomer =
            this.masterDataCommonService.getMdCustomerInfoCache(receiptHead.getPayer());
        receiptHead.setCustomerAddress(mdCustomer.getCustomerAddress());
        receiptHead.setCustomerContact(
            mdCustomer.getMdCustomerContactList().get(0).getContactName()
                + " "
                + mdCustomer.getMdCustomerContactList().get(0).getContactTelephone());
  ...
  public String getSelectSOModal(Pages pages, SoHeadCO soHeadCO, Model model) {
    ...
    for (SoHead soHead : soHeadList) {
      ...
      soHead.setDepartmentName(
          this.hrCommonService.getHrDepartment(soHead.getDepartmentCode()).getDepartmentName());
    }
  ...
  public String getSelectBankModal(Pages pages, MdVendorBankCO mdVendorBankCO, Model model) {
    ...
    // 分页查询数据
    List<MdVendorBank> mdVendorBankList =
        this.mdVendorBankService.getBankListByVendorCode(pages, mdVendorBankCO);
  ...
  public String editArInvoiceHead(
      @Valid ArInvoiceHead receiptHead,
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("AR_INVOICE", headCode);
      // 删除凭证
      ...
      // 计算分录的金额
      // 获取入库行
      List<ArInvoiceLine> lineList =
          this.arInvoiceLineService.getArInvoiceLineListByHeadCode(headCode);
      ...
      // 调用自动创建方法
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode,
          new Double[] {
            voucherAmount.doubleValue(), amountSum.doubleValue(), taxAmountSum.doubleValue()
          },
          "AR_INVOICE");
