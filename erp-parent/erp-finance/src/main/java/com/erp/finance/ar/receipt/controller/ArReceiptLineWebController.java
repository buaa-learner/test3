...
public class ArReceiptLineWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ArReceiptLineService arReceiptLineService;
  ...
  public String getArReceiptLineList(Pages pages, ArReceiptLineCO arReceiptLineCO, Model model) {
    ...
    // 分页查询数据
    List<ArReceiptLine> arReceiptLineList =
        this.arReceiptLineService.getReceiptLineListByHeadCode(pages, arReceiptLineCO);
    // 循环获取发票信息
    for (ArReceiptLine arReceiptLine : arReceiptLineList) {
      // 获取发票头信息
      ArInvoiceHead arInvoiceHead =
          this.arInvoiceHeadService.getDataObject(arReceiptLine.getInvoiceHeadCode());
      // 获取发票行汇总金额
      BigDecimal[] sumAmount =
          this.arInvoiceLineService.getInvoiceLineAmountSumByHeadCode(
              arReceiptLine.getInvoiceHeadCode());
  ...
  public String getSelectArInvoiceModal(
      Pages pages, ArInvoiceHeadCO arInvoiceHeadCO, ArReceiptLineCO arReceiptLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    List<ArInvoiceHead> invoiceHeadList =
        this.arInvoiceHeadService.getDataObjects(pages, arInvoiceHeadCO);
    // 循环设置发票信息
    for (ArInvoiceHead arInvoiceHead : invoiceHeadList) {
      arInvoiceHead.setPayerName(
          this.masterDataCommonService.getCustomerMap().get(arInvoiceHead.getPayer()));
      // 设置税额
      BigDecimal[] sumAmount =
          this.arInvoiceLineService.getInvoiceLineAmountSumByHeadCode(
              arInvoiceHead.getInvoiceHeadCode());
    ...
    List<ArReceiptLine> receiptLineList =
        this.arReceiptLineService.getReceiptLineListByHeadCode(pagesTemp, arReceiptLineCO);
    ...
    if (arReceiptLine != null && arReceiptLine.getReceiptLineId() != null) {
      ...
      BigDecimal[] sumAmount =
          this.arInvoiceLineService.getInvoiceLineAmountSumByHeadCode(
              arInvoiceHead.getInvoiceHeadCode());
      // 获取发票已核销金额
      Double invoiceReceivedAmount =
          this.arReceiptLineService.getInvoiceReceiveAmount(
              arReceiptLine.getInvoiceHeadCode(), arReceiptLine.getReceiptLineId());
      ...
      // 获取发票已核销金额
      Double invoiceReceivedAmount =
          this.arReceiptLineService.getInvoiceReceiveAmount(arReceiptLine.getInvoiceHeadCode(), 0);
      arReceiptLine.setInvoiceReceivedAmount(invoiceReceivedAmount);
    }
  ...
  @ResponseBody
  public String editArReceiptLine(
      @Valid ArReceiptLine arReceiptLine,
