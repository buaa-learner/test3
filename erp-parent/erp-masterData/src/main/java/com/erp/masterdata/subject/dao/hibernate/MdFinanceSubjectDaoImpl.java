...
public class MdFinanceSubjectDaoImpl implements MdFinanceSubjectDao {
  ...
  public List<MdFinanceSubject> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, MdFinanceSubjectCO paramObj) {
    return null;
  }
  ...
  public int getChildSubjectNum(Integer subjectId) {
    String sql =
        "select count(*) from md_finance_subject where parent_subject_code = (select d.subject_code"
            + " from md_finance_subject d where d.subject_id = :departmentId)";
