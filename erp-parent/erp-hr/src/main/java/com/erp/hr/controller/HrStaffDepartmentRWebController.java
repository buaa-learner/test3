...
public class HrStaffDepartmentRWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private HrStaffDepartmentRService hrStaffDepartmentRService;
  @Autowired private HrStaffService hrStaffService;
  @Autowired private HrPositionService hrPositionService;
  ...
  public String beforeRelateStaffDepartment(
      Pages pages, HrStaffDepartmentRCO hrStaffDepartmentRCO, Model model) {
  ...
  public String getRelateListAjax(
      Pages pages, HrStaffDepartmentRCO hrStaffDepartmentRCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    ...
    // 分页查询数据
    List relateList =
        this.hrStaffDepartmentRService.getHrStaffDepartmentRList(pages, hrStaffDepartmentRCO);
  ...
  public String getHrStaffDepartmentR(
      HrStaffDepartmentR hrStaffDepartmentR, int departmentId, Model model) {
    // 查询要编辑的数据
    if (hrStaffDepartmentR != null && hrStaffDepartmentR.getSdId() != null) {
      hrStaffDepartmentR =
          this.hrStaffDepartmentRService.getDataObject(hrStaffDepartmentR.getSdId());
    }
  ...
  public String editHrStaffDepartmentR(
      @Valid HrStaffDepartmentR hrStaffDepartmentR,
  ...
  public String deleteHrStaffDepartmentR(
      HrStaffDepartmentR hrStaffDepartmentR, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (hrStaffDepartmentR != null && hrStaffDepartmentR.getSdId() != null) {
      // 删除数据
