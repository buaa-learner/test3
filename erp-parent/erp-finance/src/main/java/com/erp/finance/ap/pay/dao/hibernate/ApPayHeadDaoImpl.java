...
public class ApPayHeadDaoImpl implements ApPayHeadDao {
  ...
  public List<ApPayHead> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, ApPayHeadCO paramObj) {
    return null;
  }
  ...
  public List<ApPayHead> getApPayHeadListForNotCreateVoucher(Pages pages, ApPayHeadCO paramObj) {
    ...
    sql =
        sql
            + " and not exists (select 1 from fin_voucher_bill_r where bill_type = 'PAY' and"
            + " bill_head_code = p.pay_head_code)";
