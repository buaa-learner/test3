...
public class InvInputHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private InvInputHeadService invInputHeadService;
  @Autowired private InvInputLineService invInputLineService;
  ...
  public String getInvInputHeadList(Pages pages, InvInputHeadCO invInputHeadCO, Model model) {
    ...
    // 分页查询数据
    List<InvInputHead> invInputHeadList =
        this.invInputHeadService.getDataObjectsForDataAuth("", pages, invInputHeadCO);
  ...
  public String getInvInputHead(InvInputHead invInputHead, Model model) {
    ...
    if (invInputHead != null && StringUtils.isNotBlank(invInputHead.getInputHeadCode())) {
      ...
      // 设置显示字段
      invInputHead.setStaffName(
          this.hrCommonService.getHrStaff(invInputHead.getStaffCode()).getStaffName());
      invInputHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(invInputHead.getDepartmentCode())
              .getDepartmentName());
      // 仓库地址
      invInputHead.setWarehouseAddress(
          warehouseMap.get(invInputHead.getWarehouseCode()).getWarehouseAddress());
      // 获取采购订单信息
      if (invInputHead.getInputSourceType().equals("PO")) {
        ...
        // 获取供应商信息
        MdVendor mdVendor =
            this.masterDataCommonService.getMdVendorInfoCache(poHead.getVendorCode());
        if (mdVendor != null) {
          ...
          if (mdVendorContactList.size() > 0) {
            invInputHead.setVendorContactDesc(
                mdVendorContactList.get(0).getContactName()
                    + "；电话"
                    + mdVendorContactList.get(0).getContactTelephone());
          }
        }
      }
  ...
  public String editInvInputHead(
      @Valid InvInputHead invInputHead,
  ...
  public String updateApproveStatus(String code, String approveStatus, RedirectAttributes attr) {
    ...
    if (StringUtils.isNotBlank(code) && StringUtils.isNotBlank(approveStatus)) {
      if (approveStatus.equals("UNSUBMIT")) {
        ...
        boolean invoiceFlag =
            this.apInvoiceHeadService.isExistApInvoiceRelatePO(
                invInputHead.getInputSourceHeadCode());
        if (invoiceFlag) {
          // 提示信息
          attr.addFlashAttribute("hint", "hint");
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("INPUT", headCode);
      // 删除凭证
      ...
      // 计算分录的金额
      // 获取入库行
      List<InvInputLine> inputLineList =
          this.invInputLineService.getInvInputLineListByInputHeadCode(headCode);
      ...
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode, new Double[] {voucherAmount.doubleValue()}, "INPUT");
