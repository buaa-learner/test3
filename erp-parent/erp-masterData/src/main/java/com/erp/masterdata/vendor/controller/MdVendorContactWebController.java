...
public class MdVendorContactWebController extends ControllerSupport {
  ...
  public String getMdVendorContactList(
      Pages pages, MdVendorContactCO mdVendorContactCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    List<MdVendorContact> mdVendorContactList =
        this.mdVendorContactService.getContactListByVendorCode(pages, mdVendorContactCO);
  ...
  @ResponseBody
  public String editMdVendorContact(
      @Valid @RequestBody MdVendorContact mdVendorContact,
  ...
  @ResponseBody
  public String deleteMdVendorContact(
      @RequestBody MdVendorContact mdVendorContact, RedirectAttributes attr) {
    try {
      // 删除数据前验证数据
      if (mdVendorContact != null && mdVendorContact.getContactId() != null) {
        // 删除数据
