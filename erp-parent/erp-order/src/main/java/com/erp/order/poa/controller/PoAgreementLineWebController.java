...
public class PoAgreementLineWebController extends ControllerSupport {
  ...
  public String getPoAgreementLineList(
      Pages pages, PoAgreementLineCO poAgreementLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    if (poAgreementLineCO.getVersion() != null && poAgreementLineCO.getVersion() > 0) {
      ...
      List<PoAgreementLine> list =
          this.poAgreementLineService.getPoLineListByPoHeadCode(pages, poAgreementLineCO);
  ...
  @ResponseBody
  public String editPoAgreementLine(
      @Valid @RequestBody PoAgreementLine poAgreementLine,
  ...
  @ResponseBody
  public String deletePoAgreementLine(
      @RequestBody PoAgreementLine poAgreementLine, RedirectAttributes attr) {
    try {
      // 删除数据前验证数据
      if (poAgreementLine != null && poAgreementLine.getPoLineId() != null) {
        // 删除数据
