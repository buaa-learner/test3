...
public class MdMaterialCategoryDaoImpl implements MdMaterialCategoryDao {
  ...
  public List<MdMaterialCategory> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, MdMaterialCategoryCO paramObj) {
    return null;
  }
  ...
  public int getChildMaterialCategoryNum(Integer categoryId) {
    String sql =
        "select count(*) from md_material_category where parent_category_code = (select"
            + " d.category_code from md_material_category d where d.category_id = :categoryId)";
