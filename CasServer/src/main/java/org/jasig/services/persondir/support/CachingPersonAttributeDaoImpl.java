...
/**
 * A configurable caching implementation of {@link IPersonAttributeDao} which caches results from a
 * wrapped IPersonAttributeDao. <br>
 * <br>
 * Configuration:
 *
 * <table border="1">
...
public class CachingPersonAttributeDaoImpl extends AbstractDefaultAttributePersonAttributeDao
  ...
  protected static final Set<IPersonAttributes> NULL_RESULTS_OBJECT =
      Collections.singleton((IPersonAttributes) new SingletonPersonImpl());
  ...
  /**
   * The Map to use for caching results. Only get, put and remove are used so the Map may be backed
   * by a real caching implementation.
   *
  ...
  /**
   * Used to specify the placeholder object to put in the cache for null results. Defaults to a
   * minimal Set. Most installations will not need to set this.
   *
  ...
  public void afterPropertiesSet() throws Exception {
    if (this.cacheKeyGenerator == null) {
      final AttributeBasedCacheKeyGenerator cacheKeyGenerator =
  ...
  /**
   * Wraps the call to the specified cachedPersonAttributesDao IPersonAttributeDao delegate with a
   * caching layer. Results are cached using keys generated by {@link #getCacheKey(Map)}.
   *
   * @see
  ...
  public Set<IPersonAttributes> getPeopleWithMultivaluedAttributes(Map<String, List<Object>> seed) {
    ...
    if (cacheKey != null) {
      ...
      if (cacheResults != null) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Retrieved query from cache for "
                  + beanName
                  + ". key='"
                  + cacheKey
                  + "', results='"
    ...
    final Set<IPersonAttributes> queryResults =
        this.cachedPersonAttributesDao.getPeopleWithMultivaluedAttributes(seed);
    ...
    if (cacheKey != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved query from wrapped IPersonAttributeDao and stored in cache for "
                + beanName
                + ". key='"
                + cacheKey
                + "', results='"
  ...
  public void removeUserAttributes(Map<String, Object> seed) {
    final Map<String, List<Object>> multiSeed =
        MultivaluedPersonAttributeUtils.toMultivaluedMap(seed);
  ...
  private static class PersonAttributeDaoMethodInvocation implements MethodInvocation {
    ...
    static {
      try {
        getPeopleWithMultivaluedAttributesMethod =
            IPersonAttributeDao.class.getMethod("getPeopleWithMultivaluedAttributes", Map.class);
      } catch (SecurityException e) {
        final NoSuchMethodError nsme =
            new NoSuchMethodError(
                "The 'getPeopleWithMultivaluedAttributes("
                    + Map.class
                    + ")' method on the '"
                    + IPersonAttributeDao.class
                    + "' is not accessible due to a security policy.");
        nsme.initCause(e);
        throw nsme;
    ...
    public AccessibleObject getStaticPart() {
      throw new UnsupportedOperationException(
          "This is a fake MethodInvocation, getStaticPart() is not supported.");
    }
    ...
    public Object getThis() {
      throw new UnsupportedOperationException(
          "This is a fake MethodInvocation, getThis() is not supported.");
    }
    ...
    public Object proceed() throws Throwable {
      throw new UnsupportedOperationException(
          "This is a fake MethodInvocation, proceed() is not supported.");
    }
  }
