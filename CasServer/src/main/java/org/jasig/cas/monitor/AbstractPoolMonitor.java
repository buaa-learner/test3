...
public abstract class AbstractPoolMonitor extends AbstractNamedMonitor<PoolStatus> {
  ...
  /**
   * Set the maximum amount of time wait while validating pool resources. If the pool defines a
   * minumum time to wait for a resource, this property should be set less than that value.
   *
  ...
  /**
   * Performs a health check on a the pool. The recommended implementation is to obtain a pool
   * resource, validate it, and return it to the pool.
   *
