...
/**
 * /** Implementation of {@link IPersonAttributeDao} that allows other subsystems and components to
 * <i>push</i> attributes to the <code>IPersonAttributeDao</code> stack. The collection of pushed
 * attributes is represented by the <code>descriptors</code> property and backed by an instance of
 * {@link AdditionalDescriptors}. In most cases this property should be configured as a
 * Session-Scoped Proxy Bean. <br>
 * <br>
 * Configuration:
 *
 * <table border="1">
...
public class AdditionalDescriptorsPersonAttributeDao
  ...
  /**
   * Called by Spring DI to inject the collection of additional descriptors. Descriptors are user
   * specific, and (therefore) the <code>Map</code> must be a session-scoped bean.
   */
  @Required
  ...
  /**
   * Sets the {@link ICurrentUserProvider} to use when determining if the additional attributes
   * should be returned, this is an optional property.
   */
  ...
  public Set<IPersonAttributes> getPeopleWithMultivaluedAttributes(
    ...
    if (targetName == null) {
      ...
      if (targetName == null) {
        this.logger.warn(
            "AdditionalDescriptors has a null name and a null name was returned by the"
                + " currentUserProvider, returning null. "
                + this.descriptors);
        return null;
      }
    }
    ...
    if (uid.equals(targetName)) {
      ...
      final IPersonAttributes personAttributes =
          new CaseInsensitiveNamedPersonImpl(targetName, this.descriptors.getAttributes());
