...
public class ApInvoiceLineDaoImpl implements ApInvoiceLineDao {
  ...
  public List<ApInvoiceLine> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, ApInvoiceLineCO paramObj) {
    return null;
  }
  ...
  public BigDecimal getHISApInvoiceAmountForPO(String poHeadCode, String payHeadCode) {
    String sql =
        "select sum(l.amount) from ap_invoice_line l where l.invoice_source_line_code in (select"
            + " pl.po_line_code from po_line pl where pl.po_head_code = :poheadcode) and"
            + " l.invoice_head_code <> :payheadcode";
  ...
  public BigDecimal getApInvoiceAmountByHeadCode(String headCode) {
    String sql =
        "select sum(l.amount) from ap_invoice_line l where l.invoice_head_code = :payheadcode";
  ...
  public BigDecimal[] getInvoiceLineAmountSumByHeadCode(String headCode) {
    String sql =
        "select sum(amount) as amount,sum(tax_amount) as tax_amount from ap_invoice_line l where"
  ...
  public Double getMadeInvoiceQuantityByPoLine(String poLineCode, Integer invoiceLineId) {
    String sql =
        "select sum(l.quantity) from ap_invoice_line l where l.invoice_source_line_code = :code and"
