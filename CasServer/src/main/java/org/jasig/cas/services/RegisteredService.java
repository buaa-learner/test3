...
public interface RegisteredService extends Cloneable, Serializable {
   ...
   * Anonymous access should not return any identifying information such as user id.
   *
  ...
  /**
   * The numeric identifier for this service. Implementations are expected to initialize the id with
   * the value of {@link #INITIAL_IDENTIFIER_VALUE}.
   *
   ...
   *     <ul>
   *       <li><code>String</code> representing the name of the attribute to consume as username
   *       <li><code>null</code> indicating the default username
   *     </ul>
   */
  String getUsernameAttribute();

  /**
   * Gets the set of handler names that must successfully authenticate credentials in order to
   * access the service. An empty set indicates that there are no requirements on particular
   * authentication handlers; any will suffice.
   *
  ...
  /**
   * An instance of the attribute filter that imposes validation rules over the attribute release
   * policy.
   *
