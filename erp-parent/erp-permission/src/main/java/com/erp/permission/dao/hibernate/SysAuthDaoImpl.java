...
public class SysAuthDaoImpl implements SysAuthDao {
  ...
  public List<SysAuth> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, SysAuthCO paramObj) {
    return null;
  }
  ...
  public List<SysAuth> getRelateSysAuthListByRoleCode(String roleCode) {
    String sql =
        "select a.* from sys_role_auth_r ra,sys_auth a where ra.auth_code=a.auth_code and"
            + " ra.role_code=:roleCode order by a.auth_code";
