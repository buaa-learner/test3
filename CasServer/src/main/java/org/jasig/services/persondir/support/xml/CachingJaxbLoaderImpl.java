...
/**
 * Base logic for loading unmarshalling an XML document via JAXB and only reloading the cached
 * object model when needed. The class attempts to monitor the lastModified date of the {@link
 * Resource} to determine when to reload. If that fails the resource is reloaded periodically as
 * specified by the {@link #setNoLastModifiedReloadPeriod(long)} property.
 *
 * <p>The class determines the return type and the base package to use for the {@link
 * JAXBContext#newInstance(String)} call via the loadedType parameter provided to the constructor.
 *
...
public class CachingJaxbLoaderImpl<T> implements CachingJaxbLoader<T> {
  ...
  protected boolean isCacheValid(Long lastModified) {
    return (lastModified != null && lastModified <= this.lastModifiedTime)
        || (lastModified == null
            && (this.lastModifiedTime + this.noLastModifiedReloadPeriod)
  ...
  protected InputStream getXmlInputStream() {
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to open InputStream for Resource: " + this.mappedXmlResource, e);
    }
  ...
  protected JAXBContext getJAXBContext() {
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to create " + JAXBContext.class + " to unmarshal " + this.loadedType, e);
    }
  }
  ...
  protected T unmarshal(final InputStream xmlInputStream, final Unmarshaller unmarshaller) {
    try {
      ...
      throw new RuntimeException(
          "Unexpected JAXB error while unmarshalling  " + this.mappedXmlResource, e);
    }
  }
}
