...
public class InvStockWebController extends ControllerSupport {
  ...
  public String getInvStockList(Pages pages, InvStockCO invStockCO, Model model) {
    ...
    for (InvStock invStock : invStockList) {
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(invStock.getMaterialCode());
  ...
  public String getMaterialInfoAjax(String materialCode) {
    ...
    if (mdMaterial != null) {
      return "{\"materialName\": \""
          + mdMaterial.getMaterialName()
          + "\",\"materialUnit\": \""
          + (StringUtils.isBlank(mdMaterial.getMaterialUnit())
              ? ""
              : materialUnitMap.get(mdMaterial.getMaterialUnit()))
          + "\",\"materialStandard\": \""
          + mdMaterial.getStandard()
          + "\"}";
    }
  ...
  public String editInvStock(
      @Valid InvStock invStock, BindingResult bindingResult, Model model, RedirectAttributes attr) {
    if (!this.invStockService.isExistInitDataForInvStock(
        invStock.getWarehouseCode(), invStock.getMaterialCode())) {
      // 参数校验
      ...
      // 备注
      invStock.setMemo(
          "库存初始化" + (StringUtils.isBlank(invStock.getMemo()) ? "" : "，" + invStock.getMemo()));
  ...
  public String deleteInvStock(InvStock invStock, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (invStock != null
        && invStock.getStockId() != null
        && StringUtils.isNotBlank(invStock.getWarehouseCode())
        && StringUtils.isNotBlank(invStock.getMaterialCode())) {
      if (this.invStockService.isExistRelateDataForInvStock(
          invStock.getWarehouseCode(), invStock.getMaterialCode())) {
        // 提示信息
        attr.addFlashAttribute("hint", "hint");
  ...
  public String getInvStockDetailList(Pages pages, InvStockCO invStockCO, Model model) {
    ...
    if (invStockCO != null && StringUtils.isNotBlank(invStockCO.getMaterialCode())) {
      // 获取物料信息
      MdMaterial mdMaterial =
          this.masterDataCommonService.getMdMaterialInfoCache(invStockCO.getMaterialCode());
      // 获取物料库存
      Double stockNumber =
          this.invStockService.getStockNumberByMaterialCode(invStockCO.getMaterialCode());
      // 分页查询数据
