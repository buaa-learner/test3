...
/**
 * Resolves principals by querying a data source using the Jasig <a
 * href="http://developer.jasig.org/projects/person-directory/1.5.0-SNAPSHOT/apidocs/">Person
 * Directory API</a>. The {@link org.jasig.cas.authentication.principal.Principal#getAttributes()}
 * are populated by the results of the query and the principal ID may optionally be set by proving
 * an attribute whose first non-null value is used; otherwise the credential ID is used for the
 * principal ID.
 *
...
public class PersonDirectoryPrincipalResolver implements PrincipalResolver {
  ...
  private IPersonAttributeDao attributeRepository =
      new StubPersonAttributeDao(new HashMap<String, List<Object>>());
  ...
  public final Principal resolve(final Credential credential) {
    RememberMeUsernamePasswordCredential rupcCredential =
    ...
    for (final String key : attributes.keySet()) {
      ...
      if (key.equalsIgnoreCase(this.principalAttributeName)) {
        if (values.isEmpty()) {
          logger.debug(
              "{} is empty, using {} for principal", this.principalAttributeName, principalId);
        } else {
  ...
  /**
   * Extracts the id of the user from the provided credential. This method should be overridded by
   * subclasses to achieve more sophisticated strategies for producing a principal ID from a
   * credential.
   *
