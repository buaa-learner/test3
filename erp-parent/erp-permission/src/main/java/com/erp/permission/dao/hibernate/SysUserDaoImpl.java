...
public class SysUserDaoImpl implements SysUserDao {
    ...
    sql =
        sql
            + DaoUtil.getSQLConditionForDateTime(
                paramObj, "createdDate", "createdDateStart", "createdDateEnd", "and u.", args);
  ...
  public List<SysUser> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, SysUserCO paramObj) {
    return null;
  }
  ...
  public List<SysUserRO> getSysUserROListForRelate() {
    String sql =
        "select u.*,(select count(*) from sys_user_role_r ur where ur.username=u.username) as"
            + " user_role_num from sys_user u order by u.user_id desc";
  ...
  public boolean isExistRelateDataForSysUser(String username) {
    String sql =
        "select (select count(*) from hr_staff s where s.username = :username)+(select count(*)"
            + " from sys_user_role_r ur where ur.username = :username) as num from dual";
