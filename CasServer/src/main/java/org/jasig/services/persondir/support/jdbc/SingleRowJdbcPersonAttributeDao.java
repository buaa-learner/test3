...
/**
 * An {@link org.jasig.services.persondir.IPersonAttributeDao} implementation that maps from column
 * names in the result of a SQL query to attribute names. <br>
 * You must set a Map from column names to attribute names and only column names appearing as keys
 * in that map will be used. <br>
 * <br>
 * Configuration:
 *
 * <table border="1">
...
public class SingleRowJdbcPersonAttributeDao
  ...
  private static final ParameterizedRowMapper<Map<String, Object>> MAPPER =
      new ColumnMapParameterizedRowMapper(true);
  ...
  protected List<IPersonAttributes> parseAttributeMapFromResults(
    ...
    for (final Map<String, Object> queryResult : queryResults) {
      final Map<String, List<Object>> multivaluedQueryResult =
          MultivaluedPersonAttributeUtils.toMultivaluedMap(queryResult);
      ...
      if (queryUserName != null) {
        ...
        person =
            new CaseInsensitiveAttributeNamedPersonImpl(userNameAttribute, multivaluedQueryResult);
      }
