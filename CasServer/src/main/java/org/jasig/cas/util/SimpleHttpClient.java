...
public final class SimpleHttpClient implements HttpClient, Serializable, DisposableBean {
  ...
  /**
   * Note that changing this executor will affect all httpClients. While not ideal, this change was
   * made because certain ticket registries were persisting the HttpClient and thus getting
  ...
  /**
   * Sends a message to a particular endpoint. Option of sending it without waiting to ensure a
   * response was returned.
   *
   * <p>This is useful when it doesn't matter about the response as you'll perform no action based
   * on the response.
   *
  ...
  public boolean sendMessageToEndPoint(
      final String url, final String message, final boolean async) {
    final Future<Boolean> result =
        EXECUTOR_SERVICE.submit(
            new MessageSender(
                url, message, this.readTimeout, this.connectionTimeout, this.followRedirects));
  ...
  /**
   * Set the acceptable HTTP status codes that we will use to determine if the response from the URL
   * was correct.
   *
  ...
  /**
   * Set the SSL socket factory be used by the URL when submitting request to check for URL endpoint
   * validity.
   *
  ...
  /**
   * Set the hostname verifier be used by the URL when submitting request to check for URL endpoint
   * validity.
   *
  ...
  private static final class MessageSender implements Callable<Boolean> {
    ...
    public Boolean call() throws Exception {
      ...
      try {
        ...
        LOGGER.warn(
            "Error Sending message to url endpoint [{}]. Error is [{}]", url, e.getMessage());
        return false;
      } finally {
