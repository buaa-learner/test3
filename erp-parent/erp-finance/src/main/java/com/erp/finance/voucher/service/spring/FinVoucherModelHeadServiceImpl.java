...
public class FinVoucherModelHeadServiceImpl implements FinVoucherModelHeadService {
  ...
  // 注入Dao
  @Autowired private FinVoucherModelHeadDao finVoucherModelHeadDao;
  @Autowired private FinVoucherModelLineDao finVoucherModelLineDao;
  ...
  public void deleteDataObject(FinVoucherModelHead obj) {
    ...
    this.finVoucherModelLineDao.deleteFinVoucherModelLineByVoucherHeadCode(
        obj.getVoucherHeadCode());
  }
  ...
  @Override
  public List<Map<String, Object>> getDataObjectsArray(
      Pages pages, FinVoucherModelHeadCO paramObj) {
    return this.finVoucherModelHeadDao.getDataObjectsArray(pages, paramObj);
  }

  @Override
  public List<FinVoucherModelHead> getDataObjectsForDataAuth(
      String dataAuthSQL, Pages pages, FinVoucherModelHeadCO paramObj) {
    return this.finVoucherModelHeadDao.getDataObjectsForDataAuth(dataAuthSQL, pages, paramObj);
  }

  @Override
  public void insertOrUpdateFinVoucherModel(
      FinVoucherModelHead finVoucherModelHead, List<FinVoucherModelLine> finVoucherModelLineList) {
    // 编辑头
  ...
  public String autoCreateVoucher(String billHeadCode, Double amount, String businessType) {
    // 获取凭证模板头
    FinVoucherModelHead finVoucherModelHead =
        this.finVoucherModelHeadDao.getFinVoucherModelHeadByBusinessType(businessType);

    // 获取凭证模板行
    if (finVoucherModelHead != null) {
      List<FinVoucherModelLine> finVoucherModelLineList =
          this.finVoucherModelLineDao.getFinVoucherModelLineListByVoucherHeadCode(
      ...
      if (finVoucherModelLineList.size() > 0) {
        ...
        finVoucherHead.setVoucherNumber(
            FinVoucherUtil.incrVoucherNumberCache(finVoucherModelHead.getVoucherType()).toString());
        // 获取当前用户职员信息
        ...
        // 返回json数据
        return "{\"errCode\":0, \"errMsg\": \"\", \"voucherHeadId\": "
