...
public interface CachingJaxbLoader<T> {
  ...
  /**
   * @param callback Callback that will be executed after the object is unmarshalled (if it needs to
   *     be) but before it is returned
  ...
  public interface UnmarshallingCallback<T> {
     ...
     * be propegated to the caller of {@link
     * JaxbLoader#getUnmarshalledObject(UnmarshallingCallback)}.
     */
