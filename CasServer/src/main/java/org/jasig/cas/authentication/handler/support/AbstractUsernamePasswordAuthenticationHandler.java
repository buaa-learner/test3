...
/**
 * Abstract class to override supports so that we don't need to duplicate the check for
...
public abstract class AbstractUsernamePasswordAuthenticationHandler
   ...
   */
  @NotNull private PasswordEncoder passwordEncoder = new PlainTextPasswordEncoder();
   ...
   *
   * @param transformedCredential the credential object bearing the transformed username and
   *     password.
   * @return HandlerResult resolved from credential on authentication success or null if no
   *     principal could be resolved from the credential.
   ...
   *
   * @param credential the credential on which the authentication was successfully performed. Note
   *     that this credential instance may be different from what was originally provided as
   *     transformation of the username may have occurred, if one is in fact defined.
   * @param principal the resolved principal
   * @param warnings the warnings
   * @return the constructed handler result
   */
  protected final HandlerResult createHandlerResult(
      final Credential credential, final Principal principal, final List<Message> warnings) {
