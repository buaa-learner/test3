...
/**
 * The regex filter that is responsible to make sure only attributes that match a certain regex
 * pattern registered service are released.
 *
...
public class RegisteredServiceRegexAttributeFilter implements RegisteredServiceAttributeFilter {
   ...
   * {@inheritDoc}
   *
   * <p>Given attribute values may be an extension of {@link Collection}, {@link Map} or an array.
   *
   * <ul>
   *   <li>The filtering operation is non-recursive.
   *   <li>Multi-valued attributes such as those of type {@link Collection} and {@link Map} are
   *       expected to allow casting to <code>Map&lt;String, String&gt;</code> or <code>
   *       Collection&lt;String&gt;</code>. Values that are of type array are expected to allow
   *       casting to <code>String[]</code>.
   *   <li>Multi-valued attributes are always put back into the final released collection of
   *       attributes as <code>String[]</code>.
   *   <li>If the final filtered collection is empty, it will not be put into the collection of
   *       attributes.
   * </ul>
   */
  @Override
    ...
    for (final String attributeName : givenAttributes.keySet()) {
      ...
      if (attributeValue != null) {
        if (attributeValue instanceof Collection) {
          ...
          final String[] filteredAttributes =
              filterArrayAttributes((String[]) attributeValue, attributeName);
          if (filteredAttributes.length > 0) {
            attributesToRelease.put(attributeName, filteredAttributes);
          }
  ...
  private Map<String, String> filterMapAttributes(final Map<String, String> valuesToFilter) {
    final Map<String, String> attributesToFilter =
        new HashMap<String, String>(valuesToFilter.size());
  ...
  private String[] filterArrayAttributes(
      final String[] valuesToFilter, final String attributeName) {
  ...
  private void logReleasedAttributeEntry(final String attributeName, final String attributeValue) {
    logger.debug(
        "The attribute value [{}] for attribute name {} matches the pattern {}. Releasing"
            + " attribute...",
        attributeValue,
        attributeName,
