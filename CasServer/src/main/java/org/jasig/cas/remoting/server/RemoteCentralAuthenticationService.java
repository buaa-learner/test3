...
/**
 * Wrapper implementation around a CentralAuthenticationService that does completes the marshalling
 * of parameters from the web-service layer to the service layer. Typically the only thing that is
 * done is to validate the parameters (as you would in the web tier) and then delegate to the
 * service layer.
 *
 * <p>The following properties are required:
 *
 * <ul>
 *   <li>centralAuthenticationService - the service layer we are delegating to.
 * </ul>
 *
...
public final class RemoteCentralAuthenticationService implements CentralAuthenticationService {
  ...
  @NotNull private CentralAuthenticationService centralAuthenticationService;

  /** The validators to check the Credential. */
  @NotNull private Validator validator = Validation.buildDefaultValidatorFactory().getValidator();

  /**
   * {@inheritDoc}
   *
   * @throws IllegalArgumentException if the Credentials are null or if given invalid credentials.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Destroy a TicketGrantingTicket and perform back channel logout. This has the effect of
