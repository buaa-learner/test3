...
public class SysRoleDaoImpl implements SysRoleDao {
  ...
  public List<SysRole> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, SysRoleCO paramObj) {
    return null;
  }
  ...
  public List<SysRole> getRelateSysRoleListByUsername(String username) {
    String sql =
        "select r.* from sys_user_role_r ur,sys_role r where ur.role_code=r.role_code and"
            + " ur.username=:username order by r.role_code";
  ...
  public boolean isExistRelateDataForSysRole(String roleCode) {
    String sql =
        "select (select count(*) from sys_role_auth_r ra where ra.role_code = :roleCode)+(select"
            + " count(*) from sys_user_role_r ur where ur.role_code = :roleCode) as num from dual";
