 ...
 *
 * <p>Using the JAAS Authentication Handler requires you to configure the appropriate JAAS modules.
 * You can specify the location of a jass.conf file using the following VM parameter:
 *
 * <pre>
 * -Djava.security.auth.login.config=$PATH_TO_JAAS_CONF/jaas.conf
 * </pre>
 *
 * <p>This example jaas.conf would try Kerberos based authentication, then try LDAP authentication:
 *
 * <pre>
 * CAS {
...
public class JaasAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
  ...
  @NotNull private String realm = DEFAULT_REALM;

  public JaasAuthenticationHandler() {
    Assert.notNull(
        Configuration.getConfiguration(),
        "Static Configuration cannot be null. Did you remember to specify"
  ...
  protected final HandlerResult authenticateUsernamePasswordInternal(
    ...
    final LoginContext lc =
        new LoginContext(
            this.realm, new UsernamePasswordCallbackHandler(username, credential.getPassword()));
    try {
  ...
  /**
   * A simple JAAS CallbackHandler which accepts a Name String and Password String in the
   * constructor. Only NameCallbacks and PasswordCallbacks are accepted in the callback array. This
   * code based loosely on example given in Sun's javadoc for CallbackHandler interface.
   */
