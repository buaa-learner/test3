...
public class InvOutputHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private InvOutputHeadService invOutputHeadService;
  @Autowired private InvOutputLineService invOutputLineService;
  ...
  @Autowired private InvWarehouseService invWarehouseService;
  ...
  public String getInvOutputHeadList(Pages pages, InvOutputHeadCO invOutputHeadCO, Model model) {
    ...
    // 分页查询数据
    List<InvOutputHead> invOutputHeadList =
        this.invOutputHeadService.getDataObjectsForDataAuth("", pages, invOutputHeadCO);
  ...
  public String getInvOutputHead(InvOutputHead invOutputHead, Model model) {
    ...
    if (invOutputHead != null && StringUtils.isNotBlank(invOutputHead.getOutputHeadCode())) {
      ...
      // 设置显示字段
      invOutputHead.setStaffName(
          this.hrCommonService.getHrStaff(invOutputHead.getStaffCode()).getStaffName());
      invOutputHead.setDepartmentName(
          this.hrCommonService
              .getHrDepartment(invOutputHead.getDepartmentCode())
              .getDepartmentName());
      // 仓库地址
      invOutputHead.setWarehouseAddress(
          warehouseMap.get(invOutputHead.getWarehouseCode()).getWarehouseAddress());
      // 获取采购订单信息
      if (invOutputHead.getOutputSourceType().equals("SO")) {
        ...
        MdCustomer mdCustomer =
            this.masterDataCommonService.getMdCustomerInfoCache(soHead.getCustomerCode());
        if (mdCustomer != null) {
          ...
          if (mdCustomerContactList.size() > 0) {
            invOutputHead.setCustomerContactDesc(
                mdCustomerContactList.get(0).getContactName()
                    + "；电话"
                    + mdCustomerContactList.get(0).getContactTelephone());
          }
        }
      }
  ...
  public String editInvOutputHead(
      @Valid InvOutputHead invOutputHead,
  ...
  public String updateApproveStatus(String code, String approveStatus, RedirectAttributes attr) {
    ...
    if (StringUtils.isNotBlank(code) && StringUtils.isNotBlank(approveStatus)) {
      if (approveStatus.equals("UNSUBMIT")) {
        ...
        boolean invoiceFlag =
            this.arInvoiceHeadService.isExistArInvoiceRelateSO(
                invOutputHead.getOutputSourceHeadCode());
        if (invoiceFlag) {
          // 提示信息
          attr.addFlashAttribute("hint", "hint");
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("OUTPUT", headCode);
      // 删除凭证
      ...
      // 计算分录的金额
      // 获取入库行
      List<InvOutputLine> outputLineList =
          this.invOutputLineService.getInvOutputLineListByOutputHeadCode(headCode);
      ...
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode, new Double[] {voucherAmount.doubleValue()}, "OUTPUT");
