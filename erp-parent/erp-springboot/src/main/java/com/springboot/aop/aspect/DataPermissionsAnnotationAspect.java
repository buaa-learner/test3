...
public class DataPermissionsAnnotationAspect {
  ...
  public Object doAroundAdvice(ProceedingJoinPoint jp) throws Throwable {
    ...
    Method method =
        jp.getTarget().getClass().getMethod(methodSignature.getMethod().getName(), classes);
    ...
    if (method.isAnnotationPresent(com.framework.annotation.Permissions.class)) {
      ...
      // 获取Permissions注解信息
      com.framework.annotation.Permissions permissionAnnotation =
          method.getAnnotation(com.framework.annotation.Permissions.class);
      ...
      if (permissionAnnotation.value() != null) {
        ...
        for (int i = 0; i < class3.length - 1; i++) {
          ...
          if (method.getParameterAnnotations()[i] != null
              && method.getParameterAnnotations()[i].length > 0) {
            for (int x = 0; x < method.getParameterAnnotations()[i].length; x++) {
              if (method.getParameterAnnotations()[i][x] != null) {
                String paramAnnotationName =
                    method.getParameterAnnotations()[i][x].annotationType().getName();
                if (StringUtils.isNotEmpty(paramAnnotationName)
                    && "com.framework.annotation.SqlParam".equals(paramAnnotationName)) {
          ...
          if (paramAnnotationFlag) {
            ...
            if (staffInfo != null) {
              ...
              // 判断当前人是否有数据权限
              if (com.framework.annotation.Permissions.PermissionType.DATA_AUTH.equals(
                      permissionAnnotation.value())
                  && StringUtils.isNotBlank(dataAuthType)) {
                ...
                if (dataAuthType.equals("public")) {
                  ...
                  if (StringUtils.isNotBlank(parentOrgStructure)) {
                    List<HrDepartment> list =
                        this.hrCommonService.getHrDepartmentChildList(parentOrgStructure);
                  ...
                  for (String value : orgStructureSet) {
                    argsql.append(
                        " "
                            + Permissions.PermissionSQLDepartmentCodeAlias
                            + "department_code ='"
                            + value
                            + "' or ");
                  }
                  ...
                  args[i] =
                      args[i]
                          + " "
                          + Permissions.PermissionSQLCreatedByAlias
                          + "created_by ='"
                          + username
                          + "') ";
                ...
                // 登陆人无数据权限，使用注解参数设置权限
                if (com.framework.annotation.Permissions.PermissionType.OWN.equals(
                    permissionAnnotation.value())) {
                  // 权限为个人
                  args[i] =
                      " and "
                          ...
                          + username
                          + "' ";
                } else if (com.framework.annotation.Permissions.PermissionType.COMPANY.equals(
                    permissionAnnotation.value())) {
                  // 当前功能未开放
                } else if (com.framework.annotation.Permissions.PermissionType.OTHERCOMPANY.equals(
                    permissionAnnotation.value())) {
                  // 当前功能未开放
                } else if (com.framework.annotation.Permissions.PermissionType.ALL.equals(
                    permissionAnnotation.value())) {
                  // 权限为公共
                  args[i] = "";
                } else {
