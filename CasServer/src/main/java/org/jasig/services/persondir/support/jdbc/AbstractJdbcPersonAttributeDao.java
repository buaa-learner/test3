...
public abstract class AbstractJdbcPersonAttributeDao<R>
   ...
   * @param queryTemplate Template to use for SQL query generation. Use {0} as the placeholder for
   *     where the generated portion of the WHERE clause should be inserted.
   */
  ...
  /**
   * Takes the {@link List} from the query and parses it into the {@link List} of {@link
   * IPersonAttributes} attributes to be returned.
   *
  ...
  protected abstract List<IPersonAttributes> parseAttributeMapFromResults(
      final List<R> queryResults, String queryUserName);
   ...
   */
  @Override
  protected PartialWhereClause appendAttributeToQuery(
      PartialWhereClause queryBuilder, String dataAttribute, List<Object> queryValues) {
  ...
  protected List<IPersonAttributes> getPeopleForQuery(
      PartialWhereClause queryBuilder, String queryUserName) {
    // Execute the query
    ...
    if (queryBuilder != null) {
      ...
      results =
          this.simpleJdbcTemplate.query(querySQL, rowMapper, queryBuilder.arguments.toArray());
