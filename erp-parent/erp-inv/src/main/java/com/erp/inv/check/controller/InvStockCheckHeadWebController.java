...
public class InvStockCheckHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private InvStockCheckHeadService invStockCheckHeadService;
  @Autowired private InvStockCheckLineService invStockCheckLineService;
  ...
  public String getInvStockCheckHeadList(
      Pages pages, InvStockCheckHeadCO invStockCheckHeadCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<InvStockCheckHead> invStockCheckHeadList =
        this.invStockCheckHeadService.getDataObjects(pages, invStockCheckHeadCO);
    ...
    InvWarehouse invWarehouse =
        this.invWarehouseService.getDataObject(invStockCheckHeadCO.getWarehouseCode());
  ...
  public String getInvStockCheckHead(InvStockCheckHead invStockCheckHead, Model model) {
    ...
    if (invStockCheckHead != null && StringUtils.isNotBlank(invStockCheckHead.getCheckHeadCode())) {
      // 修改
      invStockCheckHead =
          this.invStockCheckHeadService.getDataObject(invStockCheckHead.getCheckHeadCode());
      // 获取行
      invStockCheckLineList =
          this.invStockCheckLineService.getInvStockCheckLineListByHeadCode(
              invStockCheckHead.getCheckHeadCode());
    } else {
      // 新增
      ...
      // 获取库存盘点基础数据
      List<InvStockData> stockList =
          this.invStockCheckHeadService.getInvStockDataForCheck(
              invStockCheckHead.getWarehouseCode());
      // 循环设置盘点列表
    ...
    // 仓库
    InvWarehouse invWarehouse =
        this.invWarehouseService.getDataObject(invStockCheckHead.getWarehouseCode());
  ...
  public String editInvStockCheckHead(
      @Valid InvStockCheckHead invStockCheckHead,
      BindingResult bindingResult,
      Integer[] checkLineId,
      String[] materialCode,
      Double[] checkBeforeQuantity,
      Double[] checkAfterQuantity,
      String[] lineMemo,
      Model model,
    ...
    // 保存编辑的数据
    this.invStockCheckHeadService.insertOrUpdateStockCheck(
        invStockCheckHead, invStockCheckLineList);
    // 提示信息
  ...
  public String deleteInvStockCheckHead(
      InvStockCheckHead invStockCheckHead, RedirectAttributes attr) {
    // 删除数据前验证数据
    if (invStockCheckHead != null
        && invStockCheckHead.getCheckHeadId() != null
        && StringUtils.isNotBlank(invStockCheckHead.getCheckHeadCode())) {
  ...
  public String updateApproveStatus(
      String code, String approveStatus, String warehouseCode, RedirectAttributes attr) {
