...
public class ArInvoiceLineDaoImpl implements ArInvoiceLineDao {
  ...
  // 注入DaoSupport工具类
  @Autowired private BasicDao daoSupport;
  ...
  public List<ArInvoiceLine> getDataObjectsForDataAuth(
      @SqlParam String dataAuthSQL, Pages pages, ArInvoiceLineCO paramObj) {
    return null;
  }
  ...
  public BigDecimal getHISArInvoiceAmountForSO(String soHeadCode, String receiptHeadCode) {
    String sql =
        "select sum(l.amount) from ar_invoice_line l where l.invoice_source_line_code in (select"
            + " pl.so_line_code from so_line pl where pl.so_head_code = :soheadcode) and"
            + " l.invoice_head_code <> :receiptheadcode";
  ...
  public BigDecimal getArInvoiceAmountByHeadCode(String headCode) {
    String sql =
        "select sum(l.amount) from ar_invoice_line l where l.invoice_head_code = :receiptheadcode";
  ...
  public BigDecimal[] getInvoiceLineAmountSumByHeadCode(String headCode) {
    String sql =
        "select sum(amount) as amount,sum(tax_amount) as tax_amount from ar_invoice_line l where"
            + " l.invoice_head_code = :headCode";
  ...
  public Double getMadeInvoiceQuantityBySoLine(String soLineCode, Integer invoiceLineId) {
    String sql =
        "select sum(l.quantity) from ar_invoice_line l where l.invoice_source_line_code = :code and"
            + " l.invoice_line_id <> :id";
