...
public class SysDatasetTypeWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private SysDatasetTypeService sysDatasetTypeService;
  ...
  public String getSysDatasetTypeList(Pages pages, SysDatasetTypeCO sysDatasetTypeCO, Model model) {
    ...
    // 分页查询数据
    List<SysDatasetType> sysDatasetTypeList =
        this.sysDatasetTypeService.getDataObjects(pages, sysDatasetTypeCO);
  ...
  public String editSysDatasetType(
      @Valid SysDatasetType sysDatasetType,
  ...
  public String deleteSysDatasetType(SysDatasetType sysDatasetType, RedirectAttributes attr) {
    ...
    if (sysDatasetType != null && sysDatasetType.getDatasetTypeId() != null) {
      // 删除数据前验证数据
      if (this.sysDatasetTypeService.isExistRelateDataForSysDatasetType(
          sysDatasetType.getDatasetTypeCode())) {
        // 提示信息
