...
public class InvStockCheckLineWebController extends ControllerSupport {
  ...
  public String getInvStockCheckLineList(
      Pages pages, InvStockCheckLineCO invStockCheckLineCO, Model model) {
    // 分页查询数据
    if (pages.getPage() == 0) {
      pages.setPage(1);
    }
    ...
    // 分页查询数据
    List<InvStockCheckLine> invStockCheckLineList =
        this.invStockCheckLineService.getDataObjects(pages, invStockCheckLineCO);
  ...
  public String editInvStockCheckLine(
      @Valid InvStockCheckLine invStockCheckLine,
  ...
  public String deleteInvStockCheckLine(
      InvStockCheckLine invStockCheckLine, RedirectAttributes attr) {
    // TODO: 删除数据前验证数据
