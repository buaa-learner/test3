...
/**
 * Class to validate the credential presented by communicating with the web server and checking the
 * certificate that is returned against the hostname, etc.
 *
 * <p>This class is concerned with ensuring that the protocol is HTTPS and that a response is
 * returned. The SSL handshake that occurs automatically by opening a connection does the heavy
...
public final class HttpBasedServiceCredentialsAuthenticationHandler
  ...
  @NotNull private HttpClient httpClient;
  ...
  public HandlerResult authenticate(final Credential credential) throws GeneralSecurityException {
    ...
    if (this.requireSecure
        && !httpCredential.getCallbackUrl().getProtocol().equals(PROTOCOL_HTTPS)) {
      logger.debug("Authentication failed because url was not secure.");
      throw new FailedLoginException(
          httpCredential.getCallbackUrl() + " is not an HTTPS endpoint as required.");
    }
  ...
  /**
   * @return true if the credential provided are not null and the credential are a subclass of (or
   *     equal to) HttpBasedServiceCredential.
   */
