...
public class ApInvoiceHeadWebController extends ControllerSupport {
  ...
  // 服务层注入
  @Autowired private ApInvoiceHeadService apInvoiceHeadService;
  @Autowired private ApInvoiceLineService apInvoiceLineService;
  ...
  @Autowired private PoHeadService poHeadService;
  @Autowired private MdVendorBankService mdVendorBankService;
  @Autowired private PoLineService poLineService;
  ...
  public String getApInvoiceHeadList(Pages pages, ApInvoiceHeadCO payHeadCO, Model model) {
    ...
    // 分页查询数据
    List<ApInvoiceHead> payHeadList =
        this.apInvoiceHeadService.getDataObjectsForDataAuth("", pages, payHeadCO);
    ...
    if (payHead != null && StringUtils.isNotBlank(payHead.getInvoiceHeadCode())) {
      ...
      if (payHead.getInvoiceSourceType().equals("PO")) {
        ...
        BigDecimal HISAmount =
            this.apInvoiceLineService.getHISApInvoiceAmountForPO(
                payHead.getInvoiceSourceHeadCode(), payHead.getInvoiceHeadCode());
        payHead.setPaySourceHeadHISAmount(HISAmount == null ? 0D : HISAmount.doubleValue());
      }
      // 获取收款人信息
      payHead.setReceiverName(
          this.masterDataCommonService.getVendorMap().get(payHead.getReceiver()));
      // 获取银行信息
      payHead.setBankName(this.datasetCommonService.getBank().get(payHead.getBankCode()));
      // 获取供应商相关信息
      try {
        MdVendor mdVendor =
            this.masterDataCommonService.getMdVendorInfoCache(payHead.getReceiver());
        payHead.setVendorAddress(mdVendor.getVendorAddress());
        payHead.setVendorContact(
            mdVendor.getMdVendorContactList().get(0).getContactName()
                + " "
                + mdVendor.getMdVendorContactList().get(0).getContactTelephone());
  ...
  public String getSelectPOModal(Pages pages, PoHeadCO poHeadCO, Model model) {
    ...
    for (PoHead poHead : poHeadList) {
      ...
      poHead.setDepartmentName(
          this.hrCommonService.getHrDepartment(poHead.getDepartmentCode()).getDepartmentName());
    }
  ...
  public String getSelectBankModal(Pages pages, MdVendorBankCO mdVendorBankCO, Model model) {
    ...
    // 分页查询数据
    List<MdVendorBank> mdCustomerBankList =
  ...
  public String editApInvoiceHead(
      @Valid ApInvoiceHead payHead,
  ...
  public String autoCreateVoucherEntry(String headCode) {
    try {
      ...
      String voucherHeadCode =
          this.finVoucherBillRService.getVoucherHeadCodeByBillHeadCode("AP_INVOICE", headCode);
      // 删除凭证
      ...
      // 计算分录的金额
      // 获取入库行
      List<ApInvoiceLine> lineList =
          this.apInvoiceLineService.getApInvoiceLineListByHeadCode(headCode);
      ...
      this.finVoucherModelHeadService.autoCreateVoucher(
          headCode,
          new Double[] {
            amountSum.doubleValue(), taxAmountSum.doubleValue(), voucherAmount.doubleValue()
          },
          "AP_INVOICE");
