...
/**
 * Generates a cache key using a hash of the {@link Method} being called and for {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getMultivaluedUserAttributes(String)} and {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getUserAttributes(String)} the {@link String}
 * uid or for {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getMultivaluedUserAttributes(Map)} and {@link
 * org.jasig.services.persondir.IPersonAttributeDao#getUserAttributes(Map)} attributes from the seed
 * {@link Map} as specified by the <code>cacheKeyAttributes</code> {@link Set}
 *
 * @author Eric Dalquist
 * @version $Revision$
 */
public class AttributeBasedCacheKeyGenerator implements CacheKeyGenerator {
  private static final Map<String, Object> POSSIBLE_USER_ATTRIBUTE_NAMES_SEED_MAP =
      Collections.singletonMap(
          "getPossibleUserAttributeNames_seedMap",
          (Object)
              new Serializable() {
                private static final long serialVersionUID = 1L;
              });
  private static final Map<String, Object> AVAILABLE_QUERY_ATTRIBUTES_SEED_MAP =
      Collections.singletonMap(
          "getAvailableQueryAttributes_seedMap",
          (Object)
              new Serializable() {
                private static final long serialVersionUID = 1L;
              });
  ...
  /**
   * If all seed attributes should be used. If true cacheKeyAttributes and defaultAttributeName are
  ...
  public Serializable generateKey(MethodInvocation methodInvocation) {
    ...
    // Use the resolved cachableMethod to determine the seed Map and then get the hash of the key
    // elements
  ...
  /**
   * Get the see {@link Map} that was passed to the {@link CachableMethod}. For {@link
   * CachableMethod}s that take {@link String} arguments this method is responsible for converting
   * it into a {@link Map} using the <code>defaultAttributeName</code>.
   *
  ...
  protected Map<String, Object> getSeed(Object[] methodArguments, CachableMethod cachableMethod) {
    ...
    switch (cachableMethod) {
      ...
      case MULTIVALUED_USER_ATTRIBUTES__STR:
        {
          ...
          seed =
              Collections.singletonMap(
                  this.defaultAttributeName, (Object) Collections.singletonList(uid));
        }
        break;
      ...
      case USER_ATTRIBUTES__STR:
        ...
        // The getPossibleUserAttributeNames has a special Map seed that we return to represent
        // calls to it
      case POSSIBLE_USER_ATTRIBUTE_NAMES:
        ...
        // The getAvailableQueryAttributes has a special Map seed that we return to represent calls
        // to it
   ...
   * the <code>cacheKeyAttributes</code> {@link Set} or if it is <code>null</code> the <code>
   * defaultAttributeName</code> is used as the key attribute.
   */
